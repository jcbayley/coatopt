...
class TestParetoFront:
    ...
    def test_pareto_front_basic_2d(self):
        ...
        # For maximization (which is what the function does):
        # All points are non-dominated as each represents a different trade-off
        points = np.array([[0, 3], [1, 2], [2, 1], [3, 0]])

        front, indices = pareto_front(points)

        # All points should be on the front for maximization
        assert front.shape[0] == 4  # All four points on the front

        # Check that all original points are in the front (order may vary)
        for point in points:
            assert any(np.allclose(front_point, point) for front_point in front)
    ...
    def test_pareto_front_3d(self):
        ...
        points = np.array(
            [
                ...
                [0, 2, 0],  # On front
                [0, 0, 2],  # On front
                [0.5, 0.5, 0.5],  # Dominated by [1,1,1]
            ]
        )

        front, indices = pareto_front(points)

        # [1,1,1] dominates [0.5,0.5,0.5], so we should have 4 points on front
        assert front.shape[0] == 4
...
class TestParetoTracker:
    ...
    def test_pareto_front_computation(self, tracker):
        ...
        # Add points - for maximization all will be non-dominated
        points = [
            np.array([0, 3]),  # Non-dominated
            np.array([1, 2]),  # Non-dominated (different trade-off)
            np.array([2, 1]),  # Non-dominated (different trade-off)
        ...
        # All points should be non-dominated in maximization + the new point
        assert front.shape[0] == 5  # All 5 points are non-dominated

        # Check that all points are in the front
        all_points = np.vstack(points + [np.array([1.5, 1.5])])
...
class TestParetoTrackerEdgeCases:
    ...
    def test_update_with_identical_points(self):
        ...
        point = np.array([1, 2])
        front1, was_updated1 = tracker.add_point(point, force_update=True)
        front2, was_updated2 = tracker.add_point(point, force_update=True)

        # The tracker doesn't deduplicate, so it will be updated
        # and the front will contain duplicate points
        assert was_updated2  # This is the actual behavior
        assert front2.shape[0] == 2  # Contains duplicates
