...
class TestObjectiveExplorationPhase:
    ...
    def test_individual_phase_first_objective(self):
        ...
        result = get_objective_exploration_phase(epoch=50, n_objectives=3, episodes_per_objective=100)
    ...
    def test_individual_phase_second_objective(self):
        ...
        result = get_objective_exploration_phase(epoch=150, n_objectives=3, episodes_per_objective=100)
    ...
    def test_individual_phase_third_objective(self):
        ...
        result = get_objective_exploration_phase(epoch=250, n_objectives=3, episodes_per_objective=100)
    ...
    def test_combination_phase(self):
        ...
        result = get_objective_exploration_phase(epoch=350, n_objectives=3, episodes_per_objective=100)
    ...
    def test_cycling_through_objectives(self):
        ...
        result = get_objective_exploration_phase(epoch=299, n_objectives=3, episodes_per_objective=100)
        assert result['phase'] == 'individual'
        assert result['target_objective'] == 2
    
    def test_different_episodes_per_objective(self):
        """Test with different episodes per objective."""
        result = get_objective_exploration_phase(epoch=75, n_objectives=2, episodes_per_objective=50)
        
        assert result['phase'] == 'individual'
        assert result['target_objective'] == 1  # Second objective
        
        # Beyond individual phase
        result = get_objective_exploration_phase(epoch=125, n_objectives=2, episodes_per_objective=50)
        assert result['phase'] == 'combination'
...
class TestObjectiveSpaceCoverage:
    ...
    def test_skewed_distribution(self):
        ...
        rewards = []
        for _ in range(20):
            rewards.append([np.random.uniform(0.8, 1.0), np.random.uniform(0.0, 0.2)])  # High obj1, low obj2
...
class TestAnnealedDirichletWeights:
    ...
    def test_basic_sampling(self):
        ...
        weights = annealed_dirichlet_weights(n_objectives=3, epoch=50, total_epochs=100, num_samples=5)
    ...
    def test_annealing_progression(self):
        ...
        early_weights = annealed_dirichlet_weights(
            n_objectives=3, epoch=10, total_epochs=1000, 
            base_alpha=0.05, final_alpha=1.0, num_samples=100
        )
        
        # Late training (should produce more uniform weights)
        late_weights = annealed_dirichlet_weights(
            n_objectives=3, epoch=900, total_epochs=1000,
            base_alpha=0.05, final_alpha=1.0, num_samples=100
        )
    ...
    def test_single_sample(self):
        ...
        weights = annealed_dirichlet_weights(n_objectives=2, epoch=50, total_epochs=100, num_samples=1)
...
class TestSmoothCycleWeights:
    ...
    def test_hold_phase(self):
        ...
        weights = smooth_cycle_weights(n_objectives, t=50, T_cycle=T_cycle, T_hold=T_hold, total_steps=1000)
        
        expected = np.array([1.0, 0.0, 0.0])
        assert np.allclose(weights, expected)
        
        # Test holding second objective
        weights = smooth_cycle_weights(n_objectives, t=150, T_cycle=T_cycle, T_hold=T_hold, total_steps=1000)
    ...
    def test_transition_phase(self):
        ...
        weights = smooth_cycle_weights(n_objectives, t=90, T_cycle=T_cycle, T_hold=T_hold, total_steps=1000)
        ...
        weights = smooth_cycle_weights(n_objectives, t=95, T_cycle=T_cycle, T_hold=T_hold, total_steps=1000)
    ...
    def test_full_cycle(self):
        ...
        weights1 = smooth_cycle_weights(n_objectives, t=50, T_cycle=T_cycle, T_hold=T_hold, total_steps=1000)
        assert np.argmax(weights1) == 0
        
        # Second objective peak
        weights2 = smooth_cycle_weights(n_objectives, t=150, T_cycle=T_cycle, T_hold=T_hold, total_steps=1000)
        assert np.argmax(weights2) == 1
        
        # Third objective peak
        weights3 = smooth_cycle_weights(n_objectives, t=250, T_cycle=T_cycle, T_hold=T_hold, total_steps=1000)
    ...
    def test_beyond_total_steps(self):
        ...
        weights = smooth_cycle_weights(
            n_objectives=3, t=1500, T_cycle=300, T_hold=80, total_steps=1000,
            random_anneal=False
        )
    ...
    def test_transfer_fraction(self):
        ...
        weights = smooth_cycle_weights(
            n_objectives, t=75, T_cycle=T_cycle, T_hold=80, total_steps=1000,
        ...
        for recent_weight in recent:
            ...
            for expected in last_three_expected:
                if np.allclose(recent_weight, expected):
                    found_match = True
                    break
            assert found_match, f"Recent weight {recent_weight} not found in last 3 expected weights"
