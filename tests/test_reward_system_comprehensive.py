...
from coatopt.environments.reward_functions.reward_system import RewardCalculator, RewardRegistry
...
class TestRewardSystemComprehensive:
    ...
    def test_reward_calculation_deterministic(self, standard_env, standard_params):
        ...
        results = []
        for _ in range(5):
            total_reward, vals, rewards = calc.calculate(env=standard_env, **standard_params)
            results.append((total_reward, vals.copy(), rewards.copy()))
        
        # All results should be identical
        base_result = results[0]
        for i, result in enumerate(results[1:], 1):
            assert abs(result[0] - base_result[0]) < 1e-12, f"Run {i} total_reward differs: {result[0]} vs {base_result[0]}"
            
            for key in base_result[1]:
                if key in result[1]:
                    assert abs(result[1][key] - base_result[1][key]) < 1e-12, f"Run {i} vals[{key}] differs"
            
            for key in base_result[2]:
                if key in result[2]:
                    assert abs(result[2][key] - base_result[2][key]) < 1e-12, f"Run {i} rewards[{key}] differs"
    ...
    def test_reward_normalization_behavior(self, standard_env):
        ...
        for key in ["reflectivity", "thermal_noise", "absorption"]:
            if key in vals_no_norm and key in vals_with_norm:
                assert abs(vals_no_norm[key] - vals_with_norm[key]) < 1e-12, f"Physical value {key} changed with normalization"
    ...
    def test_addon_system_consistency(self, standard_env, standard_params):
        ...
        addon_configs = [
            ...
            {"apply_normalisation": True, "apply_boundary_penalties": True, "apply_air_penalty": True}
        ]
        ...
        for config in addon_configs:
            ...
            assert all(np.isfinite(v) for v in vals.values() if isinstance(v, (int, float))), f"Non-finite vals with config {config}"
    ...
    def test_hypervolume_combination(self, standard_env, standard_params):
        ...
        try:
            ...
            assert "hypervolume_value" in rewards, "Missing hypervolume_value in rewards"
            assert "pareto_front_size" in rewards, "Missing pareto_front_size in rewards"
            
            print(f"Hypervolume reward: {total_reward}")
            print(f"HV value: {rewards['hypervolume_value']}")
            
        except Exception as e:
            pytest.skip(f"Hypervolume calculation failed: {e}. This may be expected if pymoo is not available.")
    ...
    def test_weight_application(self, standard_env, standard_params):
        ...
        total_equal, _, rewards_equal = calc.calculate(env=standard_env, weights=weights_equal, **standard_params)
        
        # Test with skewed weights
        weights_skewed = {"reflectivity": 10.0, "thermal_noise": 0.1, "absorption": 0.1}
        total_skewed, _, rewards_skewed = calc.calculate(env=standard_env, weights=weights_skewed, **standard_params)
        ...
        for param in ["reflectivity", "thermal_noise", "absorption"]:
            if param in rewards_equal and param in rewards_skewed:
                ...
                assert abs(rewards_equal[param] - rewards_skewed[param]) < 1e-10, f"Individual reward {param} changed with weights"
    
    def test_boundary_penalty_behavior(self, standard_params):
        """Test boundary penalty behavior."""
        env_with_bounds = Mock()
        env_with_bounds.optimise_parameters = ["reflectivity", "thermal_noise", "absorption"] 
        env_with_bounds.get_parameter_names.return_value = ["reflectivity", "thermal_noise", "absorption"]
        ...
        total_within, _, rewards_within = calc.calculate(env=env_with_bounds, **params_within)
        ...
        total_outside, _, rewards_outside = calc.calculate(env=env_with_bounds, **params_outside)
        ...
        for key in penalty_keys:
            assert rewards_outside[key] <= 0, f"Boundary penalty {key} should be non-positive"
    ...
    def test_air_penalty_behavior(self, standard_env, standard_params):
        ...
        standard_env.current_state.get_layer_count.return_value = 2  # 2 air layers out of 4
        
        total_with_penalty, _, rewards_with_penalty = calc.calculate(env=standard_env, **standard_params)
        ...
        standard_env.current_state.get_layer_count.return_value = 1  # 1 air layer out of 10
        
        total_adequate, _, rewards_adequate = calc.calculate(env=standard_env, **standard_params)
        ...
        # Both should have air_penalty key
        assert "air_penalty" in rewards_with_penalty
        assert "air_penalty" in rewards_adequate
    ...
    def test_reward_function_types(self, standard_env, standard_params):
        ...
        for func_type in function_types:
            try:
                ...
                assert np.isfinite(total_reward), f"Non-finite reward with function {func_type}"
    ...
    def test_consistency_with_main_branch_config(self):
        ...
        typical_params = [
            ...
            {"reflectivity": 0.999, "thermal_noise": 1e-20, "absorption": 0.005, "thickness": 3.0},
            {"reflectivity": 0.9999, "thermal_noise": 8e-21, "absorption": 0.008, "thickness": 7.0}
        ]
        
        for params in typical_params:
            total_reward, vals, rewards = main_branch_config.calculate(env=env, **params)
            ...
            for param in ["reflectivity", "thermal_noise", "absorption"]:
                assert param in rewards or f"{param}_reward" in rewards, f"Missing reward for {param}"
...
class TestRewardSystemRegression:
    ...
    def test_nan_inf_handling(self):
        ...
        # Test edge cases
        edge_cases = [
            {"reflectivity": 1.0, "thermal_noise": 0.0, "absorption": 0.0, "thickness": 0.0},  # Zeros
            {"reflectivity": 0.0, "thermal_noise": 1e-30, "absorption": 1e-10, "thickness": 100.0},  # Extreme values
            {"reflectivity": 0.999999999, "thermal_noise": 1e-25, "absorption": 1e-12, "thickness": 0.001},  # Very small
        ]
        ...
        for params in edge_cases:
            try:
                ...
                # Check for NaN/Inf
                assert np.isfinite(total_reward), f"Non-finite total reward for edge case {params}"
                
                for key, val in vals.items():
                    if isinstance(val, (int, float)):
                        assert np.isfinite(val), f"Non-finite vals[{key}] for edge case {params}"
                
                for key, val in rewards.items():
                    if isinstance(val, (int, float)):
                        assert np.isfinite(val), f"Non-finite rewards[{key}] for edge case {params}"
    ...
    def test_memory_consistency(self):
        ...
        assert abs(total1 - total2) < 1e-12, f"Memory effect in total reward: {total1} vs {total2}"
        
        for key in vals1:
            if key in vals2:
                assert abs(vals1[key] - vals2[key]) < 1e-12, f"Memory effect in vals[{key}]"
        
        for key in rewards1:
            if key in rewards2:
                assert abs(rewards1[key] - rewards2[key]) < 1e-12, f"Memory effect in rewards[{key}]"
