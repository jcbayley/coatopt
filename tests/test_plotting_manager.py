...
class TestTrainingPlotManager:
    ...
    def test_historical_pareto_data_storage(self, basic_plot_manager):
        ...
        assert np.array_equal(basic_plot_manager.historical_pareto_data[episode], pareto_points)
    ...
    def test_ui_mode_vs_cli_mode(self, temp_dir):
        ...
        cli_manager = TrainingPlotManager(save_plots=True, output_dir=temp_dir, ui_mode=False)
...
class TestTrainingPlotManagerDataHandling:
    ...
    def test_large_data_volumes(self, plot_manager):
        ...
        for i in range(1000):
            training_point = {
                ...
                'individual_rewards': {'obj1': np.random.random(), 'obj2': np.random.random()},
    ...
    def test_objective_info_consistency(self, plot_manager):
        ...
        plot_manager.objective_targets = {'reflectivity': 0.9999, 'thermal_noise': 1e-20}
        ...
        assert len(plot_manager.objective_labels) == len(plot_manager.optimisation_parameters)
        assert len(plot_manager.objective_targets) == len(plot_manager.optimisation_parameters)
