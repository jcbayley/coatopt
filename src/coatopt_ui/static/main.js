...
const initialMaterialParams = {
    ...
    3: { name: "aSi", desc: "amorphous silicon - room temperature - Ion Planting", n: 3.73, k: 0.005, a: 10.0, alpha: 2e-6, beta: 1e-5, kappa: 30.0, C: 2000000, Y: 147000000000, prat: 0.23, phiM: 2e-5 },
...
function renderStackEditor() {
    ...
    layersState.forEach((layer, idx) => {
        ...
        let badgeClass = 'asi';
        let badgeText = `Material ${layer.material}`;
        if (layer.material === 1) {
            badgeClass = 'silica';
            badgeText = 'Silica (1)';
        } else if (layer.material === 2) {
            badgeClass = 'germania';
            badgeText = 'Germania (2)';
        } else if (layer.material === 3) {
            badgeClass = 'asi';
            badgeText = 'aSi (3)';
        }
        ...
        tr.innerHTML = `
            <td>${idx + 1}</td>
            <td>
                <span class="subsystem-badge ${badgeClass}">
                    ${badgeText}
                </span>
            </td>
...
function setupEventListeners() {
    ...
    editThickText.addEventListener("input", (e) => {
        ...
        // Sync with table row input
        const rowInput = document.querySelector(`#stackTableBody tr[data-index="${selectedLayerIndex}"] .table-thick-input`);
        if (rowInput) {
            rowInput.value = val;
        }
...
function drawPlots(data, lambda, targetLambda) {
    ...
    // --- 1. Spectrum Plot ---
    let wavelengths = data.charts.spectrum.wavelengths;
    if (wavelengths && wavelengths.length > 0 && Math.max(...wavelengths) > 1000000) {
        wavelengths = wavelengths.map(w => w / 1e9);
    }
