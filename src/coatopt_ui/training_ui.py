...
class TrainingMonitorUI:
    def __init__(self, root):
        ...
        self.click_event_connection = (
            None  # Store event connection to prevent multiple handlers
        )
    ...
    def setup_ui(self):
        ...
        self.config_entry = ttk.Entry(
            config_frame, textvariable=self.config_var, width=60
        )
        ...
        self.retrain_checkbox = ttk.Checkbutton(
            button_frame,
            text="Retrain (ignore existing data)",
        ...
        self.n_eval_samples_entry = ttk.Entry(
            eval_frame, textvariable=self.n_eval_samples_var, width=8
        )
    ...
    def setup_plots(self):
        ...
        self.rewards_fig = Figure(
            figsize=(12, 10), dpi=100
        )  # Larger figure for subplots
        ...
        self.epoch_slider = tk.Scale(
            self.pareto_controls_frame,
            from_=0,
            to=100,
            orient=tk.HORIZONTAL,
            variable=self.epoch_var,
            command=self.on_epoch_change,
            length=400,
            resolution=self.plot_update_interval,
        )
        ...
        self.viz_mode_combo = ttk.Combobox(
            self.viz_mode_frame,
        ...
        self.pareto_fig = Figure(
            figsize=(14, 10), dpi=100
        )  # Larger for multiple subplots
    ...
    def init_plots(self):
        ...
        self.values_axes = self.values_fig.subplots(
            2, 2
        )  # 4 subplots for physical values
    ...
    def load_configuration(self):
        ...
        try:
            ...
            continue_training = (
                not self.retrain_var.get()
            )  # If retrain is checked, don't continue
            ...
            callbacks = create_ui_callbacks(
                self.training_queue, lambda: not self.is_training
            )
            ...
            if hasattr(self.env, "pareto_front") and len(self.env.pareto_front) > 0:
                pareto_info = (
                    f" Loaded Pareto front: {len(self.env.pareto_front)} points."
                )

            self.status_var.set(
                f"Configuration loaded successfully. {len(self.materials)} materials loaded.{pareto_info}"
            )
            ...
            logging.CRITICAL(
                f"Error loading configuration: {e}, traceback: {traceback.format_exc()}"
            )
    ...
    def _connect_plot_manager_to_ui(self):
        ...
        try:
            ...
            rewards_fig, values_fig, pareto_fig, pareto_rewards_fig = (
                self.plot_manager.get_figures()
            )
    ...
    def load_historical_data(self):
        ...
        try:
            ...
            if self.plot_manager and hasattr(self.trainer, "checkpoint_manager"):
                success = self.plot_manager.load_context_data(
                ...
                if success:
                    ...
                    latest_reward = (
                        self.plot_manager.training_data[-1]["reward"]
                    ...
                    if self.plot_manager.pareto_data:
                        ...
                        pareto_front_size = len(
                            latest_pareto.get("pareto_front_values", [])
                        )
                        pareto_info = f", Pareto front: {pareto_front_size} points"

                    self.status_var.set(
                        f"Historical data loaded: {total_episodes} episodes, latest reward: {latest_reward:.4f}{pareto_info}"
                    )
                else:
    ...
    def _load_historical_data_legacy(self):
        ...
        try:
            ...
            if hasattr(self.trainer, "metrics") and not self.trainer.metrics.empty:
                print(
                    f"Loading historical training metrics with {len(self.trainer.metrics)} episodes..."
                )
                ...
                print(
                    f"Loaded {len(self.trainer.metrics)} training data points to plot manager"
                )

            # Load historical Pareto data
            if hasattr(self.trainer, "best_states") and self.trainer.best_states:
                print(
                    f"Loading {len(self.trainer.best_states)} historical best states..."
                )
                ...
                for i, best_state_entry in enumerate(self.trainer.best_states):
                    ...
                    if (i + 1) % self.plot_update_interval == 0 or i == len(
                        self.trainer.best_states
                    ) - 1:
                        episode = (
                            i + 1
                        ) * self.plot_update_interval  # Approximate episode number
                        pareto_data = self._generate_pareto_data_from_states(
                            accumulated_states, episode
                        )
                ...
                # Update slider range
                if (
                    hasattr(self, "historical_pareto_data")
                    and self.historical_pareto_data
                ):
                    max_episode = max(self.historical_pareto_data.keys())
                    self.epoch_slider.config(to=max_episode)
                    self.epoch_var.set(max_episode)
                    ...
                    print(
                        f"Loaded historical Pareto data for {len(self.historical_pareto_data)} time points"
                    )
    ...
    def _generate_pareto_data_from_states(self, best_states, episode):
        ...
        if hasattr(self, "optimisation_parameters") and self.optimisation_parameters:
            ...
            hasattr(self, "env")
            and self.env
            and hasattr(self.env, "optimise_parameters")
        ):
            objectives = self.env.optimise_parameters
        else:
            ...
            for obj in objectives:
                if obj in vals:
                    ...
                    if obj == "reflectivity":
                        ...
                        plot_values.append(
                            val
                        )  # Use as-is for absorption, thermal_noise, thickness
                else:
        ...
        if len(best_vals_list) > 0:
            ...
            for i, obj in enumerate(objectives):
                if obj == "reflectivity":
                    minimization_objectives[:, i] = (
                        1 - vals_array[:, i]
                    )  # Minimize (1-R)
                else:
                    minimization_objectives[:, i] = vals_array[
                        :, i
                    ]  # Minimize directly
        ...
        return {
            "episode": episode,
            "pareto_front": (
                np.array(recomputed_pareto_front)
                if recomputed_pareto_front
                else np.array([])
            ),
            ...
            "coating_states": [
                best_state_data[i] for i in pareto_indices
            ],  # For plot manager compatibility
        }
    ...
    def update_pareto_plot_for_episode(self, episode):
        ...
        if self.plot_manager:
            ...
            if click_handler:
                ...
                if (
                    hasattr(self, "click_event_connection")
                    and self.click_event_connection is not None
                ):
                    self.pareto_canvas.mpl_disconnect(self.click_event_connection)

                # Connect new handler
                self.click_event_connection = self.pareto_canvas.mpl_connect(
                    "button_press_event", click_handler
                )
            ...
            if (
                hasattr(self, "historical_pareto_data")
                and episode in self.historical_pareto_data
            ):
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", "Please enter a valid number of evaluation samples."
                )
                return
            ...
            def evaluation_worker():
                try:
                    ...
                    self.root.after(
                        0, lambda: self.on_evaluation_complete(True, num_samples)
                    )
                    ...
                    error_msg = str(e)
                    self.root.after(
                        0, lambda: self.on_evaluation_complete(False, error_msg)
                    )
    ...
    def on_evaluation_complete(self, success, result):
        ...
        if success:
            self.status_var.set(
                f"Evaluation completed successfully with {result} samples."
            )
            messagebox.showinfo(
                "Evaluation Complete",
        ...
        try:
            ...
            if hasattr(self.trainer, "checkpoint_manager"):
                ...
                if checkpoint_info.get("exists", False):
                    checkpoint_msg = (
                        f"Using unified checkpoint: {checkpoint_info['size_mb']:.1f}MB"
                    )
            ...
            if self.is_training:
                self.training_queue.put(
                    {"type": "complete", "message": "Training completed successfully!"}
                )
    ...
    def process_training_update(self, update):
        ...
        if update["type"] == "training_data":
            ...
            if (
                self.epoch_var.get() == max_episode - self.plot_update_interval
                or self.epoch_var.get() == 0
            ):
    ...
    def update_rewards_plot(self):
        ...
        if self.plot_manager:
            ...
            if self.plot_manager.training_data:
                ...
                self.status_var.set(
                    f"Training... Episode {latest['episode']}, Reward: {latest['reward']:.4f}"
                )
    ...
    def update_pareto_plot(self):
        ...
        if self.plot_manager:
            current_episode = (
                self.epoch_var.get() if hasattr(self, "epoch_var") else None
            )
    ...
    def plot_2d_pairs(self, pareto_front, best_points, obj_labels, obj_scales, episode):
        ...
        if n_objectives == 2:
            ...
            self.coating_ax = self.pareto_fig.add_subplot(
                n_rows, n_cols, total_subplots
            )
            self.coating_ax.text(
                0.5,
                0.5,
                    ...
                    for ax, i, j, scales in self.subplot_info:
                        if (
                            ...
                            if (
                                ...
                                log_front_x = np.log10(
                                    np.maximum(pareto_front[:, i], 1e-10)
                                )
                                log_front_y = np.log10(
                                    np.maximum(pareto_front[:, j], 1e-10)
                                )
                                distances = np.sqrt(
                                ...
                                distances = np.sqrt(
                                    (pareto_front[:, i] - click_x) ** 2
                                    + (pareto_front[:, j] - click_y) ** 2
                                )

                            closest_idx = np.argmin(distances)
                            self.plot_coating_stack(
                                closest_idx, pareto_front[closest_idx]
                            )
                            break
            ...
            if self.click_event_connection is not None:
                self.pareto_canvas.mpl_disconnect(self.click_event_connection)
            self.click_event_connection = self.pareto_canvas.mpl_connect(
                "button_press_event", on_click_multi
            )

    def plot_single_2d_pair(
        self,
        ax,
        pareto_front,
        best_points,
        ...
        if best_points is not None and len(best_points) > 0:
            try:
                ...
                if len(best_points_array.shape) == 2 and best_points_array.shape[
                    1
                ] > max(i, j):
        ...
        if self.env and hasattr(self.env, "design_criteria"):
            ...
            param_i = (
                self.env.optimise_parameters[i]
                if hasattr(self.env, "optimise_parameters")
                else None
            )
            param_j = (
                self.env.optimise_parameters[j]
                if hasattr(self.env, "optimise_parameters")
                else None
            )
            ...
            if param_i in targets:
                ...
                ax.axvline(
                    target_val, color="green", linestyle="--", alpha=0.7, linewidth=1
                )
    ...
    def plot_parallel_coordinates(self, pareto_front, best_points, obj_labels, episode):
        ...
        for i in range(n_objectives):
            ...
            if col_max > col_min:
                pareto_normalised[:, i] = (pareto_front[:, i] - col_min) / (
                    col_max - col_min
                )
            else:
        ...
        if best_points is not None and len(best_points) > 0:
            try:
                ...
                if best_points_array.shape[1] == n_objectives:
                    ...
                    for i in range(n_objectives):
                        col_min, col_max = (
                            best_points_array[:, i].min(),
                            best_points_array[:, i].max(),
                        )
                        if col_max > col_min:
                            best_normalised[:, i] = (
                                best_points_array[:, i] - col_min
                            ) / (col_max - col_min)
                        else:
        ...
        ax.set_title(
            f"Parallel Coordinates - Episode {episode}\n{len(pareto_front)} Pareto Points"
        )
        ax.grid(True, alpha=0.3)

    def plot_3d_scatter(
        self, pareto_front, best_points, obj_labels, obj_scales, episode
    ):
        ...
        try:
            ...
            print(
                "Warning: mpl_toolkits.mplot3d not available, 3D plots will be disabled"
            )
        ...
        if pareto_front.shape[1] >= 3:
            ...
            if len(obj_scales) >= 3:
                log_axes = [
                    i for i, scale in enumerate(obj_scales[:3]) if scale == "log"
                ]
                if log_axes:
            ...
            ax.text(
                ...
                0.5,
                0.5,
                "Need at least 3 objectives for 3D visualization",
                transform=ax.transAxes,
                ha="center",
                va="center",
            )
    ...
    def show_training_summary(self):
        ...
        try:
            ...
            if hasattr(self.trainer, "get_training_summary"):
                ...
                # Basic training info
                summary_text += (
                    f"Checkpoint Type: {summary.get('checkpoint_type', 'unknown')}\n"
                )
                summary_text += (
                    f"Current Episode: {summary.get('current_episode', 0)}\n"
                )
                summary_text += (
                    f"Target Episodes: {summary.get('target_episodes', 0)}\n"
                )
                ...
                if "total_points_explored" in summary:
                    summary_text += (
                        f"Total Points Explored: {summary['total_points_explored']}\n"
                    )
                ...
                if "checkpoint_info" in summary:
                    ...
                    summary_text += (
                        f"  Exists: {checkpoint_info.get('exists', False)}\n"
                    )
                    if checkpoint_info.get("exists", False):
                        summary_text += (
                            f"  Size: {checkpoint_info.get('size_mb', 0):.2f} MB\n"
                        )
                        summary_text += (
                            f"  Groups: {checkpoint_info.get('groups', [])}\n"
                        )
                        if "last_updated" in checkpoint_info:
                            summary_text += (
                                f"  Last Updated: {checkpoint_info['last_updated']}\n"
                            )
                ...
                messagebox.showinfo(
                    "Training Summary",
                    "Training summary not available for this trainer version.",
                )
