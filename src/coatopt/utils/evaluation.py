...
def run_evaluation_pipeline(
    ...
    create_enhanced_pareto_plots(
                trainer, env, results, sampled_states, 
                sampled_weights, output_dir
            )
...
def create_enhanced_pareto_plots(
    ...
    try:
        ...
        objectives = env.optimise_parameters if hasattr(env, 'optimise_parameters') else []
        ...
        if hasattr(trainer, 'best_states') and trainer.best_states:
            training_data = extract_training_pareto_data(trainer.best_states, objectives)
...
def extract_training_pareto_data(best_states: List, objectives: List[str]) -> Optional[Dict[str, np.ndarray]]:
    """
...
def create_combined_pareto_plots(
    ...
    if n_objectives == 2:
        ...
        ax.scatter(eval_x, eval_y, c='blue', alpha=0.6, s=20, 
                  label=f'Evaluation Samples ({n_eval_samples})', marker='o')
        ...
        if training_data is not None:
            ...
            ax.scatter(train_x, train_y, c='red', alpha=0.8, s=40,
                      label=f'Training Pareto Front ({len(train_x)})', marker='s')
        ...
        for i in range(n_objectives):
            for j in range(i + 1, n_objectives):
                if pair_idx < len(axes):
                    ...
                    ax.scatter(eval_x, eval_y, c='blue', alpha=0.6, s=15, 
                    ...
                    if training_data is not None:
                        ...
                        ax.scatter(train_x, train_y, c='red', alpha=0.8, s=25,
                                  label='Training', marker='s')
        ...
        fig.suptitle(f'Combined Pareto Fronts: Training vs Evaluation ({n_eval_samples} samples)', fontsize=14)
        fig.tight_layout()
        fig.savefig(os.path.join(output_dir, "combined_pareto_front_multi.png"), dpi=150)
        plt.close(fig)