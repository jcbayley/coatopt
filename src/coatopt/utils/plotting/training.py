"""
Legacy plotting utilities for HPPO training.
Originally from coatopt/algorithms/plotting_utils.py

TODO: Merge functionality with main plotting.py module to reduce duplication.
"""
...
def pad_lists(
    list_of_lists: List[List[Any]], padding_value: Any = 0, max_length: int = None
) -> np.ndarray:
    """
...
def make_reward_plot(metrics: pd.DataFrame, output_dir: str) -> None:
    ...
    fig, axes = plt.subplots(
        nrows=HPPOConstants.PLOT_NROWS_REWARD, figsize=HPPOConstants.PLOT_FIGSIZE_REWARD
    )
    ...
    _plot_metric_with_smoothing(
        axes[0], metrics, "reward", "Episode number", "Reward", window_size
    )
    ...
    _plot_metric_with_smoothing(
        axes[2],
        metrics,
        "thermal_noise_reward",
        "Episode number",
        "Thermal noise reward",
        window_size,
    )
    _plot_metric_with_smoothing(
        axes[3],
    ...
    # Objective weights
    axes[7].plot(
        metrics["episode"], metrics["reflectivity_reward_weights"], label="reflectivity"
    )
    axes[7].plot(
        metrics["episode"], metrics["absorption_reward_weights"], label="absorption"
    )
    axes[7].plot(
        metrics["episode"],
        metrics["thermalnoise_reward_weights"],
        label="thermal noise",
    )
...
def make_val_plot(metrics: pd.DataFrame, output_dir: str) -> None:
    ...
    """
    fig, axes = plt.subplots(
        nrows=HPPOConstants.PLOT_NROWS_VAL, figsize=HPPOConstants.PLOT_FIGSIZE_VAL
    )
    ...
    # Physical values
    _plot_metric_with_smoothing(
        axes[1], metrics, "reflectivity", "Episode number", "Reflectivity", window_size
    )
    _plot_metric_with_smoothing(
        axes[2],
        metrics,
    ...
    _plot_metric_with_smoothing(
        axes[3], metrics, "thickness", "Episode number", "Thickness", window_size
    )
    _plot_metric_with_smoothing(
        axes[4],
        metrics,
...
def make_materials_plot(
    all_materials: List[List[List[float]]],
    n_layers: int,
    output_dir: str,
    n_materials: int,
) -> None:
...
def _plot_metric_with_smoothing(
    ...
    smoothed_episodes = (
        metrics["episode"].rolling(window=window_size, center=False).median()
    )
