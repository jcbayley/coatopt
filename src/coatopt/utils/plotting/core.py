...
class TrainingPlotManager:
    ...
    def __init__(self, save_plots: bool = True, output_dir: str = None, 
        ...
        self.coating_states = {}  # Maps episode -> list of coating states for Pareto points
    ...
    def update_pareto_plot(self, episode: int = None):
        ...
        try:
            ...
            click_handler = None
            if n_objectives == 2:
                click_handler = self._plot_2d_pareto(pareto_front, best_points, current_episode, eval_points)
            else:
    ...
    def _plot_2d_pareto(self, pareto_front, best_points, episode, eval_points = None):
        ...
        if self.ui_mode:
            ...
            return self._plot_2d_pareto_with_coating(pareto_front, best_points, episode, eval_points)
        else:
            # Standard CLI mode plotting
            return self._plot_2d_pareto_standard(pareto_front, best_points, episode, eval_points)
    
    def _plot_2d_pareto_standard(self, pareto_front, best_points, episode, eval_points = None):
    ...
    def _plot_2d_pareto_with_coating(self, pareto_front, best_points, episode, eval_points = None):
        ...
        # Plot Pareto front
        pareto_scatter = pareto_ax.scatter(pareto_front[:, 0], pareto_front[:, 1],
                                          c='red', s=40, alpha=0.8, label='Pareto Front',
        ...
        if eval_points is not None and len(eval_points) > 0:
            eval_points_array = np.array(eval_points)
            if eval_points_array.shape[1] >= 2:
                pareto_ax.scatter(eval_points_array[:, 0], eval_points_array[:, 1],
                                 c='orange', s=10, alpha=0.5, label='Evaluation Points')
        ...
        if len(pareto_front) < 20:
            try:
                ...
                sorted_front_x = pareto_front[sorted_indices, 0]
                sorted_front_y = pareto_front[sorted_indices, 1]
                pareto_ax.plot(sorted_front_x, sorted_front_y, 'r-', alpha=0.5, linewidth=1)
            except:
                pass
        ...
        coating_ax.text(0.5, 0.5, 'Click a Pareto point\nto view coating stack', 
                       transform=coating_ax.transAxes, ha='center', va='center',
        ...
        def on_pareto_click(event):
            ...
            if event.inaxes == self.pareto_ax and hasattr(self, 'current_pareto_front'):
                ...
                if event.xdata is not None and event.ydata is not None:
                    ...
                    if (len(self.objective_scales) >= 2 and 
                        self.objective_scales[0] == 'log' and 
                        self.objective_scales[1] == 'log'):
                        ...
                        distances = np.sqrt((pareto_front[:, 0] - click_x)**2 + (pareto_front[:, 1] - click_y)**2)
                    ...
                    print(f"[DEBUG] Click coordinates are None: x={event.xdata}, y={event.ydata}")
            else:
                print(f"[DEBUG] Click not on pareto axis or no pareto front data available")
    ...
    def _plot_coating_stack_for_point(self, pareto_idx: int, pareto_point: np.ndarray):
        ...
        print(f"[DEBUG] _plot_coating_stack_for_point called with idx={pareto_idx}, point={pareto_point}")
        ...
        try:
            ...
            if hasattr(self, 'current_episode') and self.current_episode in self.coating_states:
            ...
            if coating_state is not None:
                ...
                if layers:
                    ...
                    obj_info = []
                    for i, val in enumerate(pareto_point[:min(len(pareto_point), len(self.objective_labels))]):
    ...
    def _configure_axis_labels_and_scales(self, ax, i, j):
        ...
        ax.set_xlabel(self.objective_labels[i] if i < len(self.objective_labels) else f'Objective {i+1}')
        ax.set_ylabel(self.objective_labels[j] if j < len(self.objective_labels) else f'Objective {j+1}')
