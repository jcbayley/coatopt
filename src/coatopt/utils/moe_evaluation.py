...
def evaluate_moe_pareto_coverage(
    ...
    """
    print(
        f"Evaluating MoE agent with {n_solutions} solutions using '{evaluation_strategy}' strategy..."
    )
    ...
    if evaluation_strategy == "comprehensive":
        ...
        weights = _generate_expert_focused_weights(
            agent, env.optimise_parameters, n_solutions
        )
        ...
        while not done:
            ...
            with torch.no_grad():
                action_output = agent.select_action(
                    state_tensor, objective_weights=obj_weights_tensor
                )
                    ...
                    if expert_analysis and moe_aux_losses:
                        ...
                        if "discrete_gate_weights" in moe_aux_losses:
                            results["gating_weights"].append(
                                {
                                    "discrete": moe_aux_losses["discrete_gate_weights"]
                                    .cpu()
                                    .numpy(),
                                    "continuous": moe_aux_losses.get(
                                        "continuous_gate_weights", None
                                    ),
    ...
    if len(results["objective_values"]) > 0:
        ...
        print(
            f"Found {len(pareto_front)} points on Pareto front out of {len(results['states'])} evaluated"
        )
...
def _generate_comprehensive_weights(
    optimise_parameters: List[str], n_solutions: int
) -> List[np.ndarray]:
...
def _generate_expert_focused_weights(
    agent, optimise_parameters: List[str], n_solutions: int
) -> List[np.ndarray]:
...
def _analyze_expert_specialization(
    gating_weights: List[Dict],
    objective_weights: List[np.ndarray],
    ...
    for expert_id in range(n_experts):
        ...
        analysis["expert_usage_frequency"][f"expert_{expert_id}"] = np.mean(
            expert_activations
        )
    ...
    if len(analysis["expert_weight_preferences"]) > 1:
        ...
        analysis["specialization_clarity"] = (
...
def plot_moe_evaluation_results(
    results: Dict[str, Any], save_path: str = None, show_expert_analysis: bool = True
):
    """
...
def _plot_2d_pareto_results(ax, results):
    ...
    # Plot all solutions
    ax.scatter(
        obj1_vals, obj2_vals, alpha=0.6, s=20, color="lightblue", label="All Solutions"
    )
    ...
    if "pareto_indices" in results:
        ...
        ax.scatter(
            pareto_obj1, pareto_obj2, s=50, color="red", label="Pareto Front", zorder=5
        )
