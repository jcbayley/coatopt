...
class MLflowTracker:
    ...
    def _log_config_parameters(self) -> None:
        ...
        mlflow.log_param("entropy_beta_decay_length", self.config.training.entropy_beta_decay_length)
        
        # Network parameters
        mlflow.log_param("pre_network_type", self.config.network.pre_network_type)
        mlflow.log_param("discrete_policy_network_nlayer", self.config.network.n_discrete_layers)
        mlflow.log_param("continuous_policy_network_nlayer", self.config.network.n_continuous_layers)
        ...
        mlflow.log_param("discrete_hidden_size", self.config.network.discrete_hidden_size)
        mlflow.log_param("continuous_hidden_size", self.config.network.continuous_hidden_size)
    ...
    def log_plots(
        self,
        plots_dir: str,
        plot_patterns: List[str] = ["*.png", "*.pdf", "*.svg"]
    ) -> None:
        """
            ...
            for file in eval_path.rglob("*"):
                if file.is_file():
                    try:
                        ...
                        mlflow.log_artifact(str(file), artifact_path=f"evaluation/{rel_path.parent}")
                    except Exception as e:
