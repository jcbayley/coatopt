...
def generate_submit_file(
    ...
    request_gpus = config.get("condor", "request_gpus", fallback=None)
    gpu_line = f"\nrequest_gpus = {request_gpus}" if request_gpus else ""

    # Check if results_dir is absolute path (for output transfer only)
    transfer_outputs = not Path(results_dir).is_absolute()

    output_dir_absolute = Path(output_dir).resolve()
...
output = {output_dir_absolute}/logs/job_$(run_id).out
error = {output_dir_absolute}/logs/job_$(run_id).err
log = {output_dir_absolute}/logs/job_$(run_id).log

# Resource requirements
request_cpus = {request_cpus}
request_memory = {request_memory}
request_disk = {request_disk}{gpu_line}
