...
class SimpleHPPOTrainingLoop:
    ...
    def _update_environment_phase(self, episode: int):
        ...
        if env.episode_count <= env.total_warmup_episodes:
            ...
            if self.verbose and (
                env.episode_count % 100 == 1 or env.episode_count <= 5
            ):
                print(
                    f"  [WARMUP] Episode {env.episode_count}/{env.total_warmup_episodes}: Target={env.target_objective}, constraints={env.constraints}"
                )
        ...
        # Phase 2: Constrained - cycle through objectives with constraints
            ...
            level = (
                (constrained_episode - 1) // env.epochs_per_step
            ) % env.steps_per_objective

            # Which objective is target
            obj_idx = (
                (constrained_episode - 1)
                // (env.epochs_per_step * env.steps_per_objective)
            ) % len(env.optimise_parameters)
            ...
            for obj in env.optimise_parameters:
                if obj != env.target_objective:
                    ...
                    constraint_fraction = 0.5 + (
                        0.4 * level / max(1, env.steps_per_objective - 1)
                    )
    ...
    def _update_pareto_front(self, vals: dict, rewards: dict, state_array):
        ...
        if not any(
            self._is_dominated(d["vals"], vals) for d in self.pareto_front_values
        ):
    ...
    def run_episode(self, objective_weights: dict = None):
        ...
        if objective_weights is None:
            ...
            objective_weights = {
                obj: 1.0 / n_obj for obj in self.env.optimise_parameters
            }
        ...
        weights_tensor = torch.FloatTensor(weights_list).unsqueeze(
            0
        )  # [1, n_objectives]

        while not done:
            # Get observation tensor for storing in buffer
            obs_tensor = state.get_observation_tensor(
                pre_type=self.agent.config.network.pre_type
            )
            ...
            if isinstance(continuous_action, torch.Tensor):
                ...
                continuous_action_tensor = torch.FloatTensor(
                    [float(continuous_action[0])]
                )
            else:
            ...
            next_state, rewards, terminated, finished, step_reward, _, vals = (
                self.env.step(action, objective_weights=objective_weights)
            )

            # Store experience in replay buffer
            self.agent.replay_buffer.update(
                discrete_action=(
                    discrete_action_tensor.detach()
                    if isinstance(discrete_action_tensor, torch.Tensor)
                    else discrete_action_tensor
                ),
                continuous_action=(
                    continuous_action_tensor.detach()
                    if isinstance(continuous_action_tensor, torch.Tensor)
                    else continuous_action_tensor
                ),
                ...
                logprob_discrete=(
                    act_logp_d.detach()
                    if isinstance(act_logp_d, torch.Tensor)
                    else act_logp_d
                ),
                logprob_continuous=(
                    act_logp_c.detach()
                    if isinstance(act_logp_c, torch.Tensor)
                    else act_logp_c
                ),
                ...
                entropy_discrete=(
                    entropy_d.detach()
                    if isinstance(entropy_d, torch.Tensor)
                    else entropy_d
                ),
                entropy_continuous=(
                    entropy_c.detach()
                    if isinstance(entropy_c, torch.Tensor)
                    else entropy_c
                ),
    ...
    def train(
        ...
        for episode in range(total_episodes):
            ...
            if (
                hasattr(self.env, "use_constrained_training")
                and self.env.use_constrained_training
            ):
            ...
            # Run episode
            ep_reward, ep_length, vals, rewards, final_state = self.run_episode(
                objective_weights
            )
            ...
            if (
                episode > 10
                and (episode + 1) % n_episodes_per_update == 0
                and len(self.agent.replay_buffer) > 0
            ):
                ...
                policy_loss = (
                    np.mean(total_policy_losses) if total_policy_losses else 0.0
                )
                ...
                lr_discrete, lr_continuous, lr_value, beta_discrete, beta_continuous = (
                    self.agent.scheduler_step(
                        episode,
                        make_step=make_scheduler_step,
                        scheduler_active=make_scheduler_step,
                    )
                )
            ...
            if self.verbose and (episode + 1) % 10 == 0:
                ...
                recent_best = (
                    max(self.episode_rewards[-100:])
                    if len(self.episode_rewards) >= 100
                    else max(self.episode_rewards)
                )
    ...
    def plot_training_progress(self):
        ...
        if (
            ...
            for i, obj in enumerate(self.env.optimise_parameters):
                ...
                if warmup_start < len(self.episode_rewards):
                    axs[0, 0].axvline(
                        warmup_start, color="gray", linestyle="--", alpha=0.3
                    )
                    axs[0, 0].text(
                        warmup_start,
                        axs[0, 0].get_ylim()[1],
            ...
            if self.env.total_warmup_episodes < len(self.episode_rewards):
                axs[0, 0].axvline(
                    self.env.total_warmup_episodes,
                    color="red",
                    linestyle="-",
                    alpha=0.5,
                    linewidth=2,
        ...
        axs[1, 0].text(
            0.5,
            len(self.pareto_front_values) + 1,
    ...
    def plot_pareto_front(self):
        ...
        # Plot 1: Value space
        reflectivities = [
            d["vals"].get("reflectivity", 0) for d in self.pareto_front_values
        ]
        absorptions = [
            d["vals"].get("absorption", 1e-3) for d in self.pareto_front_values
        ]

        axs[0].scatter(
            absorptions, reflectivities, c="red", s=100, marker="*", zorder=5
        )
        axs[0].set_xlabel("Absorption")
        axs[0].set_ylabel("Reflectivity")
        axs[0].set_xscale("log")
        axs[0].set_title(
            f"Pareto Front (Value Space) - {len(self.pareto_front_values)} points"
        )
        ...
        pareto_rewards = [
            (d["rewards"].get("reflectivity", 0), d["rewards"].get("absorption", 0))
            for d in self.pareto_front_values
            if d.get("rewards")
        ]
        if pareto_rewards:
            ...
            axs[1].scatter(
                ...
                s=60,
                marker="*",
                label=f"Pareto front ({len(pareto_rewards)})",
                zorder=5,
                edgecolor="black",
            )
    ...
    def save_pareto_to_csv(self):
        ...
        print(
            f"Saved VALUE space Pareto front to {values_path} ({len(df_values)} points)"
        )
        ...
        if rewards_data:
            ...
            print(
                f"Saved REWARD space Pareto front to {rewards_path} ({len(df_rewards)} points)"
            )
...
def train(config_path: str):
    ...
    warmup_episodes_per_objective = parser.getint(
        "hppo", "warmup_episodes_per_objective", fallback=2000
    )
    ...
    if config.training.cycle_weights == "constrained":
        ...
        print(
            f"    Total warmup: {warmup_episodes_per_objective * len(env.optimise_parameters)} episodes"
        )
