...
def create_material_mask_from_coating_state(
    ...
    if torch.sum(mask) == 0:
        print(
            f"WARNING: Material mask resulted in no valid materials! Layer {layer_num}, Previous material: {previous_material}"
        )
    ...
    masked_probs = masked_probs / (
        torch.sum(masked_probs, dim=-1, keepdim=True) + 1e-10
    )
...
def state_to_coating_state(
    state: Union[torch.Tensor, np.ndarray, dict],
    max_layers: int = 20,
    n_materials: int = 4,
...
def debug_state_consistency(
    state: Union[torch.Tensor, "CoatingState"], layer_number: Optional[int] = None
) -> None:
    """
...
def pack_state_sequence(
    state_tensor: torch.Tensor, layer_numbers_validated: torch.Tensor
) -> torch.Tensor:
    """
    ...
    packed_input = pack_padded_sequence(
        state_tensor, sequence_lengths.cpu(), batch_first=True, enforce_sorted=False
    )
...
def format_action_output(
    ...
    discrete_val = (
        discrete_action.item() if discrete_action.numel() == 1 else discrete_action
    )
    ...
    if continuous_action.numel() == 1:
        ...
        continuous_vals = (
            continuous_action.squeeze().tolist()
            if continuous_action.dim() > 1
            else continuous_action.tolist()
        )
