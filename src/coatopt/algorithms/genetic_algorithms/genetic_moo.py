...
class GeneticTrainer:
    ...
    def __init__(self, environment, config: GeneticConfig, output_dir: str):
        """
    ...
    def _create_algorithm(self):
        ...
        if self.config.algorithm == "NSGA2":
            return NSGA2(
                ...
                crossover=SBX(
                    prob=self.config.crossover_probability,
                    eta=self.config.crossover_eta,
                ),
                mutation=PM(
                    prob=self.config.mutation_probability, eta=self.config.mutation_eta
                ),
            ...
            ref_dirs = get_reference_directions(
                "uniform", len(self.env.optimise_parameters), n_partitions=n_partitions
            )
            return NSGA3(
                pop_size=self.config.population_size,
                sampling=FloatRandomSampling(),
    ...
    def evaluate(
        ...
        # Save results
        self._save_results(
            states_data, results_data, pareto_states_data, pareto_results_data
        )
        ...
        sampled_weights = np.ones(
            (len(states_data), len(self.env.optimise_parameters))
        ) / len(self.env.optimise_parameters)

        return states_data, results_data, sampled_weights

    def _process_population_data(
        self, population: Population, n_samples: Optional[int] = None
    ...
    def _save_results(
        ...
        pareto_df.to_csv(
            os.path.join(self.output_dir, "optimized_data.csv"), index=False
        )
    ...
    def _create_results_dataframe(
        self, states: np.ndarray, results: Dict[str, np.ndarray]
    ) -> pd.DataFrame:
        ...
        for i in range(self.problem.n_var):
            ...
            for state in states:
                if i < self.env.max_layers:  # Thickness variables
                    ...
                    material_idx = (
                        np.argmax(state[layer_idx, 1:]) - 1
                    )  # -1 to account for air offset
    ...
    def _save_to_hdf5(
        ...
        with h5py.File(hdf5_path, "w") as f:
            ...
            meta_group.create_dataset(
                "population_size", data=self.config.population_size
            )
            ...
            if pareto_front_rewards and pareto_front_values:
                ...
                pareto_group.create_dataset(
                    "pareto_front_rewards",
                    data=np.array(pareto_front_rewards).T,
                    compression="gzip",
                )
                pareto_group.create_dataset(
                    "pareto_front_values",
                    data=np.array(pareto_front_values).T,
                    compression="gzip",
                )
                pareto_group.create_dataset(
    ...
    def _create_sample_coating_plots(
        self, states: np.ndarray, results: Dict[str, np.ndarray]
    ):
        ...
        for i, state_idx in enumerate(indices):
            ...
            try:
                ...
                fig, ax = plot_stack(
                    state, self.env.materials, rewards=rewards, vals=vals
                )
                fig.suptitle(f"Sample Solution {i+1}/{n_plots}")

                fig.savefig(
                    os.path.join(self.output_dir, "states", f"stack_sample_{i+1}.png"),
                    dpi=150,
                    bbox_inches="tight",
                )  # Reduced DPI
                plt.close(fig)
    ...
    def generate_solutions(
        self, n_samples: int, random_weights: bool = True
