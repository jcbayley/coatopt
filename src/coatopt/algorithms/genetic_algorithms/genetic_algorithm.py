...
class StatePool:
    ...
    def __init__(
        ...
        environment,
        n_states=50,
        states_fraction_keep=0.1,
        fraction_random_add=0.0,
    ):
        ...
        self.current_values = np.concatenate(
            [
                np.arange(len(self.current_states))[:, np.newaxis],
                -np.inf * np.ones((self.n_states, 1)),
            ],
            axis=1,
        )
    ...
    def order_states(
        ...
        for i in range(self.n_states):
            temp_stval, vals, rewards = self.environment.compute_reward(
        ...
        sorted_state_values = sorted(
            state_values, key=lambda a: (-np.nan_to_num(a[1], nan=-1001))
        )
    ...
    def evolve_state(self, state):
        ...
        new_state, rewards, terminated, finished, reward, full_action = (
            self.environment.step(
                action, state=state, layer_index=action[2], always_return_value=True
            )
        )
    ...
    def evolve_step(
        ...
        self.current_states = np.tile(
            top_states, (np.ceil(self.n_states / n_keep_states).astype(int), 1, 1)
        )[: self.n_states]
        ...
        for i in range(self.n_states):
            self.current_states[i], self.current_values[i] = self.evolve_state(
                self.current_states[i]
            )
