...
class BaseNetwork(nn.Module):
    ...
    def __init__(
        ...
        if use_hyper_networks:
            self.target_input_dim = (
                input_dim + layer_dim + material_dim
            )  # Exclude objectives
    ...
    def _build_hyper_network(
        self, hyper_hidden_dim: int, hyper_n_layers: int, n_objectives: int
    ):
    ...
    def _prepare_input(
        ...
        if self.include_layer_number and layer_number is not None:
            ...
            raise ValueError(
                "layer_number must be provided when include_layer_number=True"
            )
    ...
    def _forward_hyper_network(
        self, state_input: torch.Tensor, objective_weights: torch.Tensor
        ...
        for name, dim in self.output_dims.items():
            ...
            bias = all_params[:, param_idx : param_idx + dim].view(batch_size, 1, dim)
            param_idx += dim
...
class ContinuousPolicy(BaseNetwork):
    ...
    def __init__(
        ...
        output_dims = {"mean": output_dim_continuous, "log_std": output_dim_continuous}
        ...
        if self.multi_value_rewards:
            ...
            if values.size(-1) > 1 and objective_weights is not None:
                ...
                if objective_weights.size(-1) != values.size(-1):
                    raise ValueError(
                        f"Objective weights dimension ({objective_weights.size(-1)}) "
