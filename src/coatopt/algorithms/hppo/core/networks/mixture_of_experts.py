...
class MixtureOfExpertsGating(nn.Module):
    ...
    def forward(self, objective_weights: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
class MixtureOfExpertsBase(nn.Module):
    ...
    def _generate_sobol_sequence(self) -> List[torch.Tensor]:
        ...
        if remaining_experts > 0:
            try:
                ...
                print("Warning: scipy not available for Sobol sequence, falling back to random sampling for remaining experts")
    ...
    def get_expert_specialization_loss(self, objective_weights: torch.Tensor, gate_weights: torch.Tensor) -> torch.Tensor:
        """
        ...
        expert_regions = torch.stack([region.to(device) for region in self.expert_regions])  # (n_experts, n_objectives)
        ...
        distances = torch.cdist(
            ...
            expert_regions.unsqueeze(0).expand(batch_size, -1, -1)  # (batch_size, n_experts, n_objectives)
        ...
        specialization_loss = 50.0 * F.kl_div(
            F.log_softmax(gate_weights, dim=-1),
            target_gates,
            reduction='batchmean'
        )
    ...
    def _get_expert_activation_mask(self, objective_weights: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
        ...
        current_weights = objective_weights[0] if objective_weights.size(0) > 0 else objective_weights
        ...
        if torch.rand(1).item() < 0.01:  # 1% chance to print debug info
            ...
            print(f"Active experts: {(distances <= threshold).sum().item()}/{len(distances)}")
    ...
    def forward(self, state: torch.Tensor, objective_weights: torch.Tensor, 
        ...
        expert_outputs = self.forward_experts(state, layer_number, material, objective_weights)
        
        # Mix expert outputs based on gate weights
        mixed_output = self.mix_expert_outputs(expert_outputs, gate_weights)
        
        # Compute auxiliary losses
        specialization_loss = self.get_expert_specialization_loss(objective_weights, gate_weights)
        ...
        aux_info = {
            ...
            'total_aux_loss': specialization_loss + self.load_balancing_weight * load_balance_loss
        }
...
class MoEDiscretePolicy(MixtureOfExpertsBase):
    ...
    def __init__(
        ...
        super().__init__(
            n_experts, n_objectives, expert_specialization,
            gate_hidden_dim, gate_temperature, expert_dropout, load_balancing_weight
        )
    ...
    def forward_experts(self, state: torch.Tensor, layer_number: Optional[torch.Tensor] = None, 
    ...
    def mix_expert_outputs(self, expert_outputs: List[torch.Tensor], gate_weights: torch.Tensor) -> torch.Tensor:
        """
        ...
        mixed_probs = torch.sum(
            stacked_outputs * gate_weights.unsqueeze(-1), 
            dim=1
        )
    ...
    def forward(self, state: torch.Tensor, objective_weights: torch.Tensor,
        ...
        mixed_probs, aux_info = super().forward(state, objective_weights, layer_number, None)
...
class MoEContinuousPolicy(MixtureOfExpertsBase):
    ...
    def mix_expert_outputs(self, expert_outputs: List[Tuple[torch.Tensor, torch.Tensor]], 
                          gate_weights: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        expert_means = torch.stack([output[0] for output in expert_outputs], dim=1)  # (batch_size, n_experts, output_dim)
        expert_log_stds = torch.stack([output[1] for output in expert_outputs], dim=1)  # (batch_size, n_experts, output_dim)
        
        # Weighted mixture of means
        mixed_mean = torch.sum(
            expert_means * gate_weights.unsqueeze(-1),
            dim=1
        )
        ...
        mixed_var = torch.sum(
            expert_vars * gate_weights.unsqueeze(-1),
            dim=1
        )
    ...
    def forward(self, state: torch.Tensor, objective_weights: torch.Tensor,
               layer_number: Optional[torch.Tensor] = None, material: Optional[torch.Tensor] = None):
        """
        Forward pass with signature matching how the agent calls it.
        ...
        """
        if objective_weights is None:
            raise ValueError("MoEContinuousPolicy requires objective_weights for gating")
            
        # Call parent forward with correct argument order
        (mixed_mean, mixed_log_std), aux_info = super().forward(state, objective_weights, layer_number, material)
...
class MoEValueNetwork(MixtureOfExpertsBase):
    ...
    def __init__(
        ...
        for _ in range(n_experts):
            ...
            if n_objectives > 1:
                expert.output_layers['value'] = nn.Linear(expert.hidden_dim, n_objectives)
    ...
    def forward(self, state: torch.Tensor, layer_number: Optional[torch.Tensor] = None, 
        ...
        mixed_values, aux_info = super().forward(state, objective_weights, layer_number, None)
        
        # Apply objective weighting to get scalar value
        if mixed_values.size(-1) > 1 and objective_weights is not None:
            scalar_value = torch.sum(mixed_values * objective_weights, dim=-1, keepdim=True)
        else:
