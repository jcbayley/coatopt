...
class DiscretePolicy(nn.Module):
    ...
    def forward(self, state: torch.Tensor, layer_number: Optional[torch.Tensor] = None, objective_weights: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.use_hyper_networks:
            if objective_weights is not None:
                return self._forward_hyper_network(torch.cat(inputs, dim=-1), objective_weights)
            else:
                raise ValueError("objective_weights required for hyper-network variant")
        else:
            # For standard networks, concatenate objective_weights to input if provided
            if objective_weights is not None:
    ...
    def _forward_hyper_network(self, state_input: torch.Tensor, objective_weights: torch.Tensor) -> torch.Tensor:
        """Forward pass through hyper-network."""
        batch_size = state_input.size(0)
        
        # Generate target network parameters using hyper-network
        hyper_input = objective_weights
...
class ContinuousPolicy(nn.Module):
    ...
    def forward(self, state: torch.Tensor, layer_number: Optional[torch.Tensor] = None, objective_weights: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _forward_hyper_network(self, state_input: torch.Tensor, objective_weights: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass through hyper-network."""
        batch_size = state_input.size(0)
...
class ValueNetwork(nn.Module):
    ...
    def forward(self, state: torch.Tensor, layer_number: Optional[torch.Tensor] = None, objective_weights: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.include_layer_number and layer_number is not None:
            inputs.append(layer_number)
        # For standard networks, concatenate objective_weights if provided and not using hyper networks
