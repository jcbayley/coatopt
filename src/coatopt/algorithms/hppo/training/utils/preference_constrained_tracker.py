...
class PreferenceConstrainedTracker:
    ...
    def _get_phase1_info(self, epoch: int) -> Dict[str, Any]:
        ...
        objective_index = (
            epoch // self.phase1_epochs_per_objective
        ) % self.n_objectives
    ...
    def _get_phase2_info(self, epoch: int) -> Dict[str, Any]:
        ...
        constraints = self._get_progressive_constraints(
            constraint_step, target_objective
        )
    ...
    def _get_progressive_constraints(
        self, constraint_step: int, target_objective: str
    ) -> Dict[str, float]:
        ...
        for param in self.optimise_parameters:
            if param != target_objective:
                ...
                constraint_threshold = min_with_margin + constraint_progress * (
                    max_val - min_with_margin
                )
    ...
    def apply_constraint_penalties(
        self, rewards: Dict[str, float], constraints: Dict[str, float]
    ) -> float:
        """
    ...
    def load_from_checkpoint_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.cycle_objective_per_constraint_steps = data.get(
                "cycle_objective_per_constraint_steps", False
            )
