...
def get_objective_exploration_phase(
    epoch: int, n_objectives: int, episodes_per_objective: int = 100
) -> dict:
    """
...
def analyze_objective_space_coverage(
    all_rewards: Optional[List[np.ndarray]], n_objectives: int
) -> Dict[str, np.ndarray]:
    """
    ...
    for obj_idx in range(n_objectives):
        ...
        exploration_deficit = max(
            0, expected_per_bin - objective_densities[obj_idx][min_density_bin]
        )
...
def adaptive_weight_distribution(
    ...
    if all_rewards is None or len(all_rewards) < 50 or exploration_factor > 0.7:
        return _early_exploration_weights(
            n_objectives, archive_memory, exploration_factor
        )
...
def _early_exploration_weights(
    n_objectives: int,
    archive_memory: Optional[List[np.ndarray]],
...
def _generate_targeted_weights(
    under_explored_regions: np.ndarray, n_objectives: int
) -> np.ndarray:
...
def _apply_archive_diversity(
    weight: np.ndarray,
    archive_memory: List[np.ndarray],
    n_objectives: int,
    max_attempts: int = 10,
) -> np.ndarray:
    ...
    for attempt in range(max_attempts):
        ...
        min_distance = min(
            [np.linalg.norm(weight - archived) for archived in archive_memory]
        )
...
class WeightArchive:
    ...
    def get_recent_weights(self, n_recent: int = 10) -> List[np.ndarray]:
        ...
        return (
            self.weights[-n_recent:]
            if len(self.weights) >= n_recent
...
def annealed_dirichlet_weights(
    n_objectives, epoch, total_epochs, base_alpha=0.05, final_alpha=1.0, num_samples=1
):
    """
...
def sample_reward_weights(
    ...
    if cycle_weights == "individual_then_adaptive":
        ...
        if phase_info["phase"] == "individual":
            ...
            if epoch is not None and final_weight_epoch > 0:
                ...
                if remaining_epochs > 0:
                    progress = min(
                        (epoch - individual_phase_episodes) / remaining_epochs, 1.0
                    )
                else:
                    progress = 1.0
        ...
        if n_objectives != 2:
            ...
            warnings.warn(
                f"'step' cycle_weights is designed for 2 objectives but {n_objectives} were provided. "
        ...
        weights = smooth_cycle_weights(
            ...
            T_cycle=T_cycle,
            T_hold=T_hold,
            total_steps=final_weight_epoch,
