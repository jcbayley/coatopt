...
from coatopt.algorithms.hppo.training.checkpoint_manager import (
...
class HPPOTrainer:
    ...
    def __init__(
        ...
        if self.entropy_beta_continuous_start is not None:
            self.agent.entropy_beta_continuous_start = (
        ...
        if (
            ...
            and self.env.cycle_weights == "preference_constrained"
        ):
            self.pc_tracker = PreferenceConstrainedTracker(
                ...
                constraint_penalty_weight=getattr(
                    self.env, "pc_constraint_penalty_weight", 50.0
                ),
                constraint_margin=getattr(self.env, "pc_constraint_margin", 0.05),
                cycle_objective_per_constraint_steps=getattr(
                    self.env, "pc_cycle_objective_per_constraint_steps", False
                ),
            )
    ...
    def _setup_directories(self) -> None:
        ...
        if self.weight_network_save:
            self.network_weights_dir = os.path.join(
                self.root_dir, HPPOConstants.NETWORK_WEIGHTS_DIR
            )
    ...
    def _setup_adaptive_moe_tracking(self) -> None:
        ...
        if hasattr(self.agent, "actor") and hasattr(
            self.agent.actor, "use_mixture_of_experts"
        ):
            if self.agent.actor.use_mixture_of_experts:
                ...
                if hasattr(self.agent.actor.discrete_policy, "expert_specialization"):
                    specialization = (
                        self.agent.actor.discrete_policy.expert_specialization
                    )
                elif hasattr(
                    self.agent.actor.continuous_policy, "expert_specialization"
                ):
                    specialization = (
                        self.agent.actor.continuous_policy.expert_specialization
                    )
                ...
                if specialization == "adaptive_constraints":
                    ...
                    if config and hasattr(config, "network"):
                        self.moe_phase1_episodes = getattr(
                            config.network, "moe_phase1_episodes", 1000
                        )
                        self.moe_constraint_experts_per_obj = getattr(
                            config.network, "moe_constraint_experts_per_objective", 2
                        )
                        self.moe_constraint_penalty_weight = getattr(
                            config.network, "moe_constraint_penalty_weight", 100.0
                        )
                    else:
                        # Fallback defaults
                    ...
                    print(
                        f"Adaptive MoE enabled: Phase 1 will run for {self.moe_phase1_episodes} episodes"
                    )
    ...
    def _initialize_training_state(self) -> None:
        ...
        self.context.training_metrics = pd.DataFrame(
            columns=[
                "episode",
    ...
    def _load_unified_checkpoint(self) -> None:
        ...
        try:
            ...
            if (
                self.context.current_episode == 0
                and len(self.context.training_metrics) == 0
            ):
            ...
            if len(self.context.pareto_front_rewards) > 0:
                ...
                print(
                    f"Loaded Pareto front with {len(self.context.pareto_front_rewards)} points into centralized tracker"
                )
            ...
            print(
                f"Successfully loaded unified checkpoint from episode {self.start_episode}"
            )
    ...
    def _update_pareto_tracking(
        self, state: np.ndarray, rewards_dict: Dict, vals_dict: Dict
    ) -> None:
        """
        ...
        reward_point = np.array(
            [rewards_dict.get(param, 0.0) for param in self.env.get_parameter_names()]
        )
        value_point = np.array(
            [vals_dict.get(param, 0.0) for param in self.env.get_parameter_names()]
        )
        ...
        updated_front, was_updated = self.pareto_tracker.add_point(
            point=reward_point, values=value_point, state=state
        )

        if was_updated:
            # Update context from tracker
            self.context.pareto_front_rewards = self.pareto_tracker.get_front(
                force_update=True
            )
            self.context.pareto_front_values = self.pareto_tracker.get_values(
                force_update=True
            )
            self.context.pareto_states = self.pareto_tracker.get_states(
                force_update=True
            )

            # Update reference point
            if len(self.context.pareto_front_rewards) > 0:
                self.context.reference_point = (
                    np.max(self.context.pareto_front_rewards, axis=0) * 1.1
                )
    ...
    def train(self) -> Tuple[Dict[str, float], np.ndarray]:
        ...
        print(
            f"Starting training from episode {self.start_episode} to {self.n_iterations}"
        )
        ...
        try:
            ...
            if self.callbacks.on_training_complete:
                completion_info = {
                    ...
                    "final_episode": getattr(
                        self, "current_episode", self.n_iterations - 1
                    ),
                }
    ...
    def _run_training_loop(self) -> Tuple[Dict[str, float], np.ndarray]:
        ...
        for episode in range(self.start_episode, self.n_iterations):
            ...
            episode_metrics, episode_data, episode_reward, final_state = (
                self._run_training_episode(episode)
            )

            # Update best states and reward tracking
            self._update_episode_tracking(
                episode_data, all_means, all_stds, all_materials
            )
            max_reward, max_state = self._update_max_reward(
                episode_reward, final_state, max_reward, max_state
            )
            ...
            self._save_network_weights_if_needed(
                episode, episode_data["objective_weights"]
            )

            # Record metrics
            self._record_episode_metrics(
                episode, episode_metrics, episode_reward, final_state, episode_data
            )
            ...
            if episode % self.callbacks.summary_interval == 0 and episode != 0:
                self._perform_periodic_tasks(
                    episode,
                    all_materials[0] if all_materials else [],
                    episode_start_time,
                    start_time,
                    final_state,
                    episode_reward,
        ...
        return (
            self.context.training_metrics.iloc[-1].to_dict()
    ...
    def _handle_adaptive_moe_step(
        self, episode: int, rewards: Dict, objective_weights_tensor, action_output
    ):
        """
        ...
        if self.adaptive_moe_phase == 1:
            ...
            if expert_idx is not None and expert_idx < len(
                self.env.get_parameter_names()
            ):
                objective_name = self.env.get_parameter_names()[expert_idx]
                if (
                    objective_name in rewards
                    and objective_name in self.adaptive_moe_reward_histories
                ):
                    self.adaptive_moe_reward_histories[objective_name].append(
                        rewards[objective_name]
                    )

            # Check if we should switch to Phase 2
            if (
                episode >= self.moe_phase1_episodes
                and not self.adaptive_moe_phase_switched
            ):
        ...
        # Phase 2: Apply expert constraints
            ...
            if expert_idx is not None:
                ...
                if hasattr(self.agent.actor.discrete_policy, "get_expert_constraints"):
                    constraints = (
                        self.agent.actor.discrete_policy.get_expert_constraints(
                            expert_idx
                        )
                    )
                elif hasattr(
                    self.agent.actor.continuous_policy, "get_expert_constraints"
                ):
                    constraints = (
                        self.agent.actor.continuous_policy.get_expert_constraints(
                            expert_idx
    ...
    def _switch_to_adaptive_moe_phase2(self, episode: int):
        ...
        for obj_name, history in self.adaptive_moe_reward_histories.items():
            if history:
                ...
                print(
                    f"  {obj_name}: {len(history)} samples, range=[{min_r:.3f}, {max_r:.3f}], avg={avg_r:.3f}"
                )

        # Update MoE networks with constraint targets
        networks_to_update = []
        if hasattr(self.agent.actor, "discrete_policy") and hasattr(
            self.agent.actor.discrete_policy, "update_constraint_expert_regions"
        ):
            networks_to_update.append(self.agent.actor.discrete_policy)
        if hasattr(self.agent.actor, "continuous_policy") and hasattr(
            self.agent.actor.continuous_policy, "update_constraint_expert_regions"
        ):
            networks_to_update.append(self.agent.actor.continuous_policy)
        if hasattr(self.agent.actor, "value_network") and hasattr(
            self.agent.actor.value_network, "update_constraint_expert_regions"
        ):
    ...
    def _run_training_episode(
        ...
        for rollout in range(self.n_episodes_per_epoch):
            ...
            if rollout_data["total_reward"] > min_episode_score[0]:
                min_episode_score = (
                    rollout_data["total_reward"],
                    rollout,
                    rollout_data["final_state"],
        ...
        episode_data = {
            ...
            "objective_weights": rollout_data[
                "objective_weights"
    ...
    def _run_single_rollout(self, episode: int) -> Dict[str, Any]:
        ...
        weights_result = sample_reward_weights(
            ...
            weight_archive=(
                self.weight_archive if hasattr(self, "weight_archive") else None
            ),  # Phase 2 enhancement
            all_rewards=(
                self.context.all_rewards if self.context.all_rewards else None
            ),  # Pass all rewards for coverage analysis
        ...
        for step in range(HPPOConstants.MAX_EPISODE_STEPS):
            ...
            objective_weights_tensor = (
                torch.tensor(objective_weights).unsqueeze(0).to(torch.float32)
            )
            ...
            if self.use_adaptive_moe:
                self._handle_adaptive_moe_step(
                    episode, rewards, objective_weights_tensor, action_output
                )
        ...
        if len(multiobjective_rewards_list) > 0 and self.agent.multi_value_rewards:
            returns = self.agent.get_returns(
                rewards_list, multiobjective_rewards=multiobjective_rewards_list
            )
    ...
    def _update_episode_tracking(
        self, episode_data: Dict, all_means: List, all_stds: List, all_materials: List
    ) -> None:
    ...
    def _update_max_reward(
        self,
        episode_reward: float,
        final_state: np.ndarray,
        max_reward: float,
        max_state: Optional[np.ndarray],
    ...
    def _perform_network_updates(
        self, episode: int, episode_metrics: Dict, make_scheduler_step: bool
    ) -> None:
        ...
        if episode > HPPOConstants.DEFAULT_UPDATE_FREQUENCY:
            for update_index in range(self.n_updates_per_epoch):
                loss1, loss2, loss3 = self.agent.update(
                    update_policy=True, update_value=True
                )
            lr_outs = self.agent.scheduler_step(
                ...
                episode_metrics.update(
                    {
                        ...
                        "beta": (beta_discrete + beta_continuous)
                        / 2,  # Average for backward compatibility
            ...
            if hasattr(self.agent, "beta_discrete") and hasattr(
                self.agent, "beta_continuous"
            ):
                default_metrics.update(
                    {
                        "beta_discrete": self.agent.beta_discrete,
                        "beta_continuous": self.agent.beta_continuous,
                        "beta": (self.agent.beta_discrete + self.agent.beta_continuous)
                        / 2,
                    }
                )
            else:
    ...
    def _save_network_weights_if_needed(
        self, episode: int, objective_weights: np.ndarray
    ) -> None:
    ...
    def _record_episode_metrics(
        ...
        objective_names = (
            ...
            else ["reflectivity", "thermal_noise", "absorption"]
        )
    ...
    def _perform_periodic_tasks(
        ...
        if (
            self.save_episode_visualizations
            and episode % HPPOConstants.SAVE_INTERVAL == 0
        ):
        ...
        print(
            f"Episode {episode + 1}: Total Reward: {episode_reward:.4f}, "
    ...
    def _save_unified_checkpoint(self, episode: int, migration: bool = False) -> None:
        ...
        try:
            ...
            if episode % HPPOConstants.EPISODE_PRINT_INTERVAL == 0 or migration:
                ...
                print(
                    f"Unified {status} saved: Episode {episode}, "
    ...
    def _save_model_checkpoint(self, episode: int) -> None:
        ...
        self.agent.save_networks(
            os.path.join(self.root_dir, HPPOConstants.NETWORK_WEIGHTS_DIR),
            episode=episode,
        )
    ...
    def init_pareto_front(self, n_solutions: int = 5) -> None:
        ...
        sol_states = np.zeros(
            (n_solutions, self.env.max_layers, state_shape[1])
        )  # Assuming 2D observation shape
        ...
        for i in range(n_solutions):
            ...
            reflectivity, thermal_noise, absorption, thickness = (
                self.env.compute_state_value(state, return_separate=True)
            )
    ...
    def generate_solutions(
        self, n_solutions: int, random_weights: bool = True
    ) -> Tuple[List[np.ndarray], List[Dict], List[np.ndarray], List[Dict]]:
        """
            ...
            objweights = np.array(
                np.meshgrid(
                    *[
                        ...
                        for _ in range(self.agent.num_objectives)
                    ]
                )
            )
        ...
        for n in range(n_solutions):
            ...
            for step in range(HPPOConstants.MAX_EPISODE_STEPS):
                ...
                next_state, rewards, done, finished, _, full_action, vals = (
                    self.env.step(
                        action,
                        objective_weights=objective_weights,
                        pc_tracker=self.pc_tracker,
    ...
    def _generate_pareto_update(self, episode: int) -> Optional[Dict]:
        ...
        return {
            ...
            "episode": episode,
            "pareto_front": (
                np.array(transformed_pareto_points)
                if transformed_pareto_points
                else np.array([])
            ),
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "completion_percent": (
                round((getattr(self, "start_episode", 0) / self.n_iterations) * 100, 2)
                if self.n_iterations > 0
                else 0
            ),
            "metrics_count": len(self.metrics) if hasattr(self, "metrics") else 0,
            "best_states_count": (
                len(self.best_states) if hasattr(self, "best_states") else 0
            ),
        }
        ...
        if hasattr(self.env, "pareto_front"):
            summary["pareto_front_size"] = (
                len(self.env.pareto_front) if self.env.pareto_front is not None else 0
            )
    ...
    def backup_checkpoint(self) -> str:
        ...
        if hasattr(self, "checkpoint_manager"):
            ...
            backup_dir = os.path.join(
                os.path.dirname(self.root_dir), f"training_backup_{timestamp}"
            )
            shutil.copytree(
                self.root_dir, backup_dir, ignore=shutil.ignore_patterns("backup_*")
            )
...
def create_cli_callbacks(
    verbose: bool = True, plot_manager=None, trainer=None
) -> TrainingCallbacks:
    ...
    def cli_progress_update(episode: int, info: Dict[str, Any]):
        if verbose:
            ...
            eta_seconds = (
                (info["total_episodes"] - episode - 1) / eps_per_sec
                if eps_per_sec > 0
                else 0
            )
            eta_str = (
            ...
            print(
                f"Episode {episode:4d}/{info['total_episodes']} | "
        ...
        if plot_manager:
            ...
            if (
                trainer
                and hasattr(trainer, "context")
                and trainer.context.constraint_history
                and len(trainer.context.constraint_history) > 0
            ):
                # Add the latest constraint entry
                latest_entry = trainer.context.constraint_history[-1]
                if (
                    latest_entry["episode"] == episode
                ):  # Make sure it's for this episode
