...
def create_environment(
    config: CoatingOptimisationConfig, materials: Dict[int, Dict[str, Any]]
) -> Union[HPPOEnvironment, MultiObjectiveEnvironment, GeneticCoatingStack]:
    """
    ...
    if model_type == "hppo":
        ...
        raise ValueError(
            f"Unsupported model type: {model_type}. Supported types: hppo, multiobjective, genetic"
        )


def create_hppo_environment(
    config: CoatingOptimisationConfig, materials: Dict[int, Dict[str, Any]]
) -> HPPOEnvironment:
    """
    ...
    env = HPPOEnvironment(
        ...
        objective_bounds=(
            config.data.objective_bounds
            if hasattr(config.data, "objective_bounds")
            else None
        ),
...
def create_multiobjective_environment(
    config: CoatingOptimisationConfig, materials: Dict[int, Dict[str, Any]]
    ...
    env = MultiObjectiveEnvironment(
        ...
        start_weight_alpha=(
            config.training.start_weight_alpha if config.training else 1.0
        ),
        final_weight_alpha=(
            config.training.final_weight_alpha if config.training else 1.0
        ),
        ...
        apply_preference_constraints=(
            config.training.cycle_weights == "preference_constrained"
            if config.training
            else False
        ),
        air_penalty_weight=config.data.air_penalty_weight,
        ...
        preference_constraint_weight=(
            getattr(config.training, "pc_constraint_penalty_weight", 1.0)
            if config.training
            else 1.0
        ),
        multi_value_rewards=config.network.multi_value_rewards,
...
def create_pc_hppo_agent(
    config: CoatingOptimisationConfig,
    env: Union[HPPOEnvironment, MultiObjectiveEnvironment],
) -> hppo.PCHPPO:
    """
...
def create_trainer(
    ...
    if config.training.cycle_weights == "preference_constrained":
        env.pc_phase1_epochs_per_objective = getattr(
            config.training, "pc_phase1_epochs_per_objective", 1000
        )
        env.pc_phase2_epochs_per_step = getattr(
            config.training, "pc_phase2_epochs_per_step", 300
        )
        env.pc_constraint_steps = getattr(config.training, "pc_constraint_steps", 8)
        env.pc_constraint_penalty_weight = getattr(
            config.training, "pc_constraint_penalty_weight", 50.0
        )
        env.pc_constraint_margin = getattr(
            config.training, "pc_constraint_margin", 0.05
        )
        env.pc_cycle_objective_per_constraint_steps = getattr(
            config.training, "pc_cycle_objective_per_constraint_steps", False
        )
...
def load_model_if_needed(
    agent: hppo.PCHPPO, config: CoatingOptimisationConfig, continue_training: bool
) -> None:
    """
    ...
    if config.general.load_model or continue_training:
        if config.general.load_model_path == "root" or continue_training:
            agent.load_networks(
                os.path.join(
                    config.general.root_dir, hppo.HPPOConstants.NETWORK_WEIGHTS_DIR
                )
            )
        else:
            agent.load_networks(config.general.load_model_path)
        print(
            f"Loaded model from: {config.general.load_model_path if config.general.load_model_path != 'root' else config.general.root_dir}"
        )


def setup_optimisation_pipeline(
    config: CoatingOptimisationConfig,
    materials: Dict[int, Dict[str, Any]],
    continue_training: bool = False,
    init_pareto_front: bool = True,
) -> Tuple[
    Union[HPPOEnvironment, MultiObjectiveEnvironment], hppo.PCHPPO, hppo.HPPOTrainer
]:
    """
...
def create_genetic_environment(
    config: CoatingOptimisationConfig, materials: Dict[int, Dict[str, Any]]
) -> GeneticCoatingStack:
    """
...
def create_genetic_trainer(
    config: CoatingOptimisationConfig, env: GeneticCoatingStack
) -> GeneticTrainer:
    """
    ...
    trainer = GeneticTrainer(
        environment=env, config=config.genetic, output_dir=config.general.root_dir
    )
    return trainer


def setup_genetic_optimisation_pipeline(
    config: CoatingOptimisationConfig, materials: Dict[int, Dict[str, Any]]
) -> Tuple[GeneticCoatingStack, GeneticTrainer]:
    """
...
def setup_training_pipeline(
    ...
    if model_type == "genetic":
        ...
        return setup_optimisation_pipeline(
            config, materials, continue_training, init_pareto_front
        )
    else:
        raise ValueError(
            f"Unsupported model type: {model_type}. Supported types: hppo, multiobjective, genetic"
...
def validate_model_config(config: CoatingOptimisationConfig) -> None:
    ...
    if model_type == "genetic":
        if config.genetic is None:
            raise ValueError(
                "Genetic configuration section is required for genetic algorithms"
            )

    if model_type in ["hppo", "multiobjective", "pareto"]:
        if config.training is None:
            raise ValueError(
                "Training configuration section is required for RL algorithms"
            )
