...
class LayerData:
    ...
    def __repr__(self) -> str:
        return f"LayerData(thickness={self.thickness:.6f}, material={self.material_index})"
...
class CoatingState:
    ...
    def __init__(self, max_layers: int, n_materials: int, 
                 air_material_index: int = 0, substrate_material_index: int = 1,
                 include_electric_field: bool = False, materials: Optional[List[Dict]] = None):
        """
    ...
    @classmethod
    def from_tensor(cls, state_tensor: torch.Tensor, air_material_index: int = 0, 
        ...
        state = cls(max_layers, n_materials, air_material_index, substrate_material_index)
        state._state_tensor = state_tensor.clone()
        return state    
    
    @classmethod
    def from_layers(cls, layers: List[LayerData], max_layers: int, n_materials: int,
                   air_material_index: int = 0, substrate_material_index: int = 1) -> 'CoatingState':
        """
    ...
    @classmethod
    def load_from_array(cls, data: Union[np.ndarray, torch.Tensor, dict, List], 
                       max_layers: Optional[int] = None, n_materials: Optional[int] = None,
                       air_material_index: int = 0, substrate_material_index: int = 1) -> 'CoatingState':
        """
        ...
        if isinstance(data, dict):
            ...
            if 'tensor' in data:
                ...
                return cls.load_from_array(data['state'], max_layers, n_materials, 
                                         air_material_index, substrate_material_index)
            else:
            ...
            if data and isinstance(data[0], LayerData):
                if max_layers is None or n_materials is None:
                    raise ValueError("Must specify max_layers and n_materials for LayerData list")
                return cls.from_layers(data, max_layers, n_materials, air_material_index, substrate_material_index)
            else:
        ...
        if state_tensor.dim() == 1:
            ...
            if max_layers is None or n_materials is None:
                raise ValueError("Must specify max_layers and n_materials for 1D tensor")
            expected_size = max_layers * (n_materials + 1)
            if state_tensor.size(0) != expected_size:
                raise ValueError(f"1D tensor size {state_tensor.size(0)} doesn't match expected {expected_size}")
        ...
        return cls.from_tensor(state_tensor, air_material_index, substrate_material_index)
    ...
    def onehot_to_material_index(self, layer_idx: int) -> int:
        ...
        if layer_idx >= self.max_layers or layer_idx < 0:
            raise IndexError(f"Layer index {layer_idx} out of bounds [0, {self.max_layers})")
    ...
    def material_index_to_onehot(self, material_idx: int) -> torch.Tensor:
        ...
        if material_idx >= self.n_materials or material_idx < 0:
            raise IndexError(f"Material index {material_idx} out of bounds [0, {self.n_materials})")
    ...
    def get_material_indices_sequence(self, max_layers: Optional[int] = None, 
        ...
        if max_layers is None:
            max_layers = self.get_num_active_layers() if active_only else self.max_layers
        else:
    ...
    @classmethod
    def from_observation(cls, observation: Union[Dict, np.ndarray, torch.Tensor],
                        max_layers: Optional[int] = None, n_materials: Optional[int] = None,
                        air_material_index: int = 0, substrate_material_index: int = 1) -> 'CoatingState':
        """
    ...
    def is_valid(self) -> bool:
        ...
        return not (torch.any(torch.isinf(self._state_tensor)) or torch.any(torch.isnan(self._state_tensor)))
    ...
    def set_layer(self, layer_idx: int, thickness: float, material_index: int):
        ...
        if material_index >= self.n_materials or material_index < 0:
            raise IndexError(f"Material index {material_index} out of bounds [0, {self.n_materials})")
        ...
        if self._debug_enabled:
            print(f"DEBUG: Set layer {layer_idx} -> thickness={thickness:.6f}, material={material_index}")
    
    def invalidate_enhanced_observation_cache(self):
        """Invalidate cached enhanced observations when state changes."""
        self._enhanced_observation_dirty = True
    
    def get_enhanced_observation(self, include_field_data: bool = None,
        ...
        if (not self._enhanced_observation_dirty and 
            hasattr(self, '_enhanced_observation_cache') and 
            self._enhanced_observation_cache is not None and
            cache_key in self._enhanced_observation_cache):
        ...
        # Cache the result
        if not hasattr(self, '_enhanced_observation_cache') or self._enhanced_observation_cache is None:
    ...
    def get_observation_tensor(self, include_field_data: bool = None,
                             merit_function_callback=None,
                             pre_type: str = None,
    ...
    def _compute_electric_field_info(self, num_field_points: int = 50, 
                                   merit_function_callback=None, 
                                   materials=None, **physics_params) -> Optional[Dict]:
        """
    ...
    def get_tensor_for_network(self, include_enhanced_features: bool = False, 
                             merit_function_callback=None, **physics_params) -> torch.Tensor:
        """
        ...
        if 'electric_field' in obs and obs['electric_field'] is not None:
            ...
            if field_points != n_layers and n_layers > 1:
                efield = torch.nn.functional.interpolate(
                    efield.unsqueeze(0).unsqueeze(0), size=n_layers, mode='linear', align_corners=False
                ).squeeze()
                gradients = torch.nn.functional.interpolate(
                    gradients.unsqueeze(0).unsqueeze(0), size=n_layers, mode='linear', align_corners=False  
                ).squeeze()
            ...
            metrics_expanded = metrics.unsqueeze(0).expand(n_layers, -1)  # [n_layers, 3]
    ...
    def get_consecutive_violations(self, max_layers: Optional[int] = None) -> List[Dict]:
        """
    ...
    def get_total_thickness(self, material_index: Optional[int] = None, 
                          max_layers: Optional[int] = None) -> float:
        """
    ...
    def get_layer_count(self, material_index: Optional[int] = None,
                       max_layers: Optional[int] = None) -> int:
        """
    ...
    def validate(self, check_consecutive: bool = True, 
                check_bounds: bool = True) -> List[str]:
        """
        ...
        if check_consecutive and self.has_consecutive_materials():
            ...
            for v in violations:
                issues.append(f"  - {v['count']} consecutive material {v['material_index']} "
        ...
        if check_bounds:
            ...
            for i in range(self.max_layers):
                ...
                if layer.material_index >= self.n_materials:
                    issues.append(f"Layer {i} has invalid material index {layer.material_index}")
            
            # Check thickness values for active layers
            for i, layer in enumerate(self.get_active_layers()):
                if layer.thickness <= 0:
                    issues.append(f"Active layer {i} has non-positive thickness {layer.thickness}")
    ...
    def __str__(self) -> str:
        ...
        for i in range(active_count):
            if self._state_tensor[i, 0] > 0:
                ...
                material_name = MaterialIndex(material_idx).name if material_idx < 4 else f"MAT_{material_idx}"
                lines.append(f"  Layer {i}: {material_name} ({material_idx}), Thickness {thickness:.6f}")
        ...
        if violations:
            ...
            for v in violations:
                material_name = MaterialIndex(v['material_index']).name if v['material_index'] < 4 else f"MAT_{v['material_index']}"
                lines.append(f"  - {v['count']} consecutive {material_name} ({v['material_index']}) layers")
