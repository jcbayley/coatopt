...
from ..utils.EFI_tmm import (  # Observation space depends on whether electric field information is included
    CalculateEFI_tmm,
    optical_to_physical,
    physical_to_optical,
)
...
class BaseCoatingEnvironment:
    ...
    def _init_from_config(self, config: CoatingOptimisationConfig):
        ...
        self.include_electric_field = getattr(
        ...
        self.optimise_parameter_names = self._extract_parameter_names(
        ...
        self.use_reward_normalisation = getattr(
            config.data, "use_reward_normalisation", False
        )
        self.reward_normalisation_mode = getattr(
            config.data, "reward_normalisation_mode", "fixed"
        )
        self.reward_normalisation_ranges = getattr(
            config.data, "reward_normalisation_ranges", {}
        )
        self.reward_normalisation_alpha = getattr(
            config.data, "reward_normalisation_alpha", 0.1
        )
        self.reward_normalisation_apply_clipping = getattr(
            config.data, "reward_normalisation_apply_clipping", True
        )

        # Reward addon system configuration from DataConfig
        self.apply_boundary_penalties = getattr(
            config.data, "apply_boundary_penalties", False
        )
        self.apply_divergence_penalty = getattr(
            config.data, "apply_divergence_penalty", False
        )
        self.apply_air_penalty = getattr(config.data, "apply_air_penalty", False)
        self.apply_pareto_improvement = getattr(
            config.data, "apply_pareto_improvement", False
        )
        self.air_penalty_weight = getattr(config.data, "air_penalty_weight", 1.0)
        self.divergence_penalty_weight = getattr(
            config.data, "divergence_penalty_weight", 1.0
        )
        self.pareto_improvement_weight = getattr(
            config.data, "pareto_improvement_weight", 1.0
        )
    ...
    def get_parameter_names(self):
        ...
        if hasattr(self, "optimise_parameter_names"):
            ...
            return self._extract_parameter_names(
                getattr(self, "optimise_parameters", [])
            )

    def _init_from_legacy_params(
        self,
        max_layers=20,
        min_thickness=1e-9,
        max_thickness=1e-6,
    ...
    def _setup_common_attributes(self):
        ...
        reward_calc_config = {
            ...
            "use_reward_normalisation": getattr(
                self, "use_reward_normalisation", False
            ),
            "reward_normalisation_mode": getattr(
                self, "reward_normalisation_mode", "fixed"
            ),
            "reward_normalisation_ranges": getattr(
                self, "reward_normalisation_ranges", {}
            ),
            "reward_normalisation_alpha": getattr(
                self, "reward_normalisation_alpha", 0.1
            ),
            "reward_normalisation_apply_clipping": getattr(
                self, "reward_normalisation_apply_clipping", True
            ),
            # Addon configuration
            "apply_boundary_penalties": getattr(
                self, "apply_boundary_penalties", False
            ),
            "apply_divergence_penalty": getattr(
                self, "apply_divergence_penalty", False
            ),
            "apply_air_penalty": getattr(self, "apply_air_penalty", False),
            "apply_pareto_improvement": getattr(
                self, "apply_pareto_improvement", False
            ),
            "apply_preference_constraints": getattr(
                self, "apply_preference_constraints", False
            ),
            "air_penalty_weight": getattr(self, "air_penalty_weight", 1.0),
            "divergence_penalty_weight": getattr(
                self, "divergence_penalty_weight", 1.0
            ),
            "pareto_improvement_weight": getattr(
                self, "pareto_improvement_weight", 1.0
            ),
            "preference_constraint_weight": getattr(
                self, "preference_constraint_weight", 1.0
            ),
    ...
    def _get_observation_shape(self):
        ...
        try:
            ...
            sample_obs_tensor = sample_state.get_observation_tensor(
                ...
                merit_function_callback=(
                    self._merit_function_wrapper
    ...
    def sample_action_space(
        ...
        new_layer_material = torch.nn.functional.one_hot(
            torch.from_numpy(np.array(np.random.randint(self.n_material_options))),
            num_classes=self.n_material_options,
        )
        new_layer_thickness = (
            torch.rand(1) * (self.max_thickness - self.min_thickness)
            + self.min_thickness
        )
    ...
    def get_observation_from_state(self, state=None):
        ...
        return state.get_enhanced_observation(
            include_field_data=self.include_electric_field,
            merit_function_callback=(
                self._merit_function_wrapper if self.include_electric_field else None
            ),
    ...
    def _merit_function_wrapper(self, state_trim, materials, **kwargs):
        ...
        return coating_utils.merit_function(np.array(state_trim), materials, **kwargs)
    ...
    def get_observation_array(self, state=None):
        ...
        for layer in layer_stack:
            observation.append([layer["thickness"], layer["n"], layer["k"]])
