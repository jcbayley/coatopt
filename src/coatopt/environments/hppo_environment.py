...
class HPPOEnvironment(BaseCoatingEnvironment):
    ...
    def step(self, action, objective_weights=None, always_return_value=False):
        ...
        done = (self.current_index >= self.max_layers) or (
            material_idx == self.air_material_index and self.current_index > 1
        )

        # Calculate reward
        if done or always_return_value:
            total_reward, vals, rewards = self.compute_reward(
                self.current_state, objective_weights
            )
        else:
            total_reward = 0.0
            vals = {}
        ...
        info = {
            ...
            "done_reason": (
                "max_layers" if self.current_index >= self.max_layers else "air_layer"
            ),
