...
def reward_function_log_targets(
    reflectivity,
    thermal_noise,
    total_thickness,
    absorption,
    ...
    **kwargs,
    ...
    if "reflectivity" in optimise_parameters:
        log_reflect = -np.log(
            np.abs(reflectivity - optimise_targets["reflectivity"]) + 1e-30
        )
        rewards["reflectivity"] = log_reflect

    if "thermal_noise" in optimise_parameters and thermal_noise is not None:
        log_therm = -np.log(
            np.abs(thermal_noise - optimise_targets["thermal_noise"]) + 1e-30
        )
    ...
    if "absorption" in optimise_parameters:
        log_absorption = -np.log(
            np.abs(absorption - optimise_targets["absorption"]) + 1e-30
        )
    ...
    if combine == "sum":
        total_reward = np.sum(
            [rewards[key] * weights[key] for key in optimise_parameters]
        )
        ...
        raise ValueError(
            f"combine must be either 'sum', 'product', or 'logproduct', not {combine}"
        )
