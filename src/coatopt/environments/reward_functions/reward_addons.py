...
def apply_divergence_penalty(
    ...
    if len(optimise_parameters) == 2:
        ...
        if min_weight > 0.01:
            ...
            if min_reward < 0.1:
                divergence_penalty -= (
                    (0.1 - min_reward) * min_weight * divergence_penalty_weight
                )
            ...
            if len(optimise_parameters) > 0 and multi_value_rewards:
                ...
                for param in optimise_parameters:
                    updated_rewards[param] = (
                        updated_rewards.get(param, 0.0) + penalty_per_objective
                    )
        else:
    ...
    updated_rewards["total_reward"] = (
        updated_rewards.get("total_reward", 0.0) + updated_rewards["divergence_addon"]
    )
    return updated_rewards


def calculate_air_penalty_reward_new(
    state,
    air_material_index: int = 0,
    design_criteria: Dict = None,
    current_vals: Dict = None,
    optimise_parameters: List[str] = None,
    penalty_strength: float = 1.0,
    reward_strength: float = 1.0,
    min_real_layers: int = 5,
) -> float:
    """
    ...
    if hasattr(state, "get_layer_count"):
        ...
        non_air_layers = total_layers - state.get_layer_count(
            material_index=air_material_index
        )
        air_fraction = (
            state.get_layer_count(material_index=air_material_index) / total_layers
            if total_layers > 0
            else 1.0
        )
        ...
        air_fraction = (
            1.0 - (non_air_layers / total_layers) if total_layers > 0 else 1.0
        )
            ...
            if key in current_vals and key in optimise_parameters:
                ...
                if key in ["reflectivity"]:
                    ...
                    if val < threshold:
                        criteria_met[key] = True
                        break
    criteria_met = all(criteria_met.values()) if len(criteria_met) > 0 else False
...
def apply_pareto_improvement_addon(
    total_reward: float,
    rewards: Dict[str, float],
    vals: Dict[str, float],
    env,
    ...
    **kwargs,
    ...
    if (
        env is not None
        and hasattr(env, "pareto_tracker")
        and env.pareto_tracker is not None
        and optimise_parameters is not None
        and len(optimise_parameters) >= 2
    ):

        try:
            # Extract objective values for the optimized parameters
            new_point = np.array(
                [rewards.get(param, 0.0) for param in optimise_parameters]
            )
            new_value = np.array(
                [vals.get(param, 0.0) for param in optimise_parameters]
            )
            ...
            _, was_updated = test_tracker.add_point(
                new_point, new_value, new_state, force_update=True
            )
    ...
    if optimise_parameters and len(optimise_parameters) > 0 and multi_value_rewards:
        ...
        for param in optimise_parameters:
            updated_rewards[param] = (
                updated_rewards.get(param, 0.0) + reward_per_objective
            )

    # Store original Pareto improvement reward for debugging
    updated_rewards["pareto_improvement_addon"] = (
        pareto_improvement_reward * pareto_improvement_weight
    )

    return updated_rewards


def apply_preference_constraints_addon(
    total_reward: float,
    rewards: Dict[str, float],
    vals: Dict[str, float],
    env,
    ...
    **kwargs,
    ...
    if pc_tracker is None or phase_info is None:
        raise Exception(
            "PreferenceConstrainedTracker and phase_info must be provided for preference constraints addon."
        )
    ...
    if current_phase == 2:
        ...
        if constraints:
            ...
            constraint_penalty = pc_tracker.apply_constraint_penalties(
                rewards, constraints
            )
            ...
            updated_rewards["pc_target_objective"] = phase_info.get(
                "target_objective", "unknown"
            )
        else:
