...
def re_integrand(
    ...
    for i, layer in enumerate(state):
        ...
        layer_depths = np.linspace(
            start_depth, end_depth, num_points_layer, endpoint=False
        )
    ...
    if current_total_points != num_points:
        ...
        final_layer_depths = np.linspace(
            cumulative_thickness[-2],
            cumulative_thickness[-1],
            adjustment + len(depths[-adjustment:]),
            endpoint=False,
        )
...
def integrand(EFI, light_wavelength, materialLayer, materialParams, num_points=30000):
    ...
    layer_thicknesses = [
        light_wavelength / (4 * materialParams[mat]["n"]) for mat in materialLayer
    ]
    cumulative_thickness = np.cumsum(layer_thicknesses)
    ...
    for i, thickness in enumerate(layer_thicknesses):
        ...
        layer_ref_indices = np.full(
            layer_depths.shape, materialParams[materialLayer[i]]["n"]
        )
    ...
    if current_total_points != num_points:
        ...
        final_layer_ref_indices = np.full(
            final_layer_depths.shape, materialParams[materialLayer[-1]]["n"]
        )
        depths[-adjustment:] = final_layer_depths
        ref_indices[-adjustment:] = final_layer_ref_indices
...
def merit_function(
    ...
    if use_optical_thickness:
        ...
        layer_thicknesses = np.array(
            [
                optical_to_physical(
                    layer_optical_thicknesses[i],
                    light_wavelength,
                    all_materials[layer_material_inds[i]]["n"],
                )
                for i in range(len(layer_optical_thicknesses))
            ]
        )
    else:
        layer_thicknesses = state[:, 0]
        layer_optical_thicknesses = np.array(
            [
                physical_to_optical(
                    layer_thicknesses[i],
                    light_wavelength,
...
def merit_function_2(
    ...
    layer_optical_thicknesses = np.array(
        [
            physical_to_optical(layer_thicknesses[i], light_wavelength, ns[i])
            for i in range(len(layer_thicknesses))
        ]
    )
