...
def get_optimal_state(
    ...
    for i in range(max_layers):
        current_material = (
            inds_alternate[1] if material == inds_alternate[0] else inds_alternate[0]
        )
...
def trim_state(state, air_material_index=0):
    ...
    if isinstance(state, CoatingState):
        ...
        return (
            np.array(trimmed_state)
            if trimmed_state
            else np.array([[0] + [0] * state.n_materials])
        )
...
def tensor_to_coating_state(
    tensor: torch.Tensor, air_material_index: int = 0, substrate_material_index: int = 1
):
...
def array_to_coating_state(
    array: np.ndarray, air_material_index: int = 0, substrate_material_index: int = 1
):
...
def material_list_to_state(
    material_indices, thicknesses, max_layers, n_materials, air_material_index=0
):
    """
