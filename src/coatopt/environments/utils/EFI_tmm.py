...
def CalculateEFI_tmm(
    ...
    n_sub = materialParams[substrate_index][
        "n"
    ...
    if plots:
        ...
        for i in range(len(materialLayer)):
            ...
            if depBreak is not None and i == depBreak - 1:
                ...
                print(
                    f"Layer before break - Position: {before_position}, Material: {before_layer['name']}, Thickness: {before_thickness}"
                )
                ...
                if i + 1 < len(materialLayer):
                    ...
                    print(
                        f"Layer after break - Position: {after_position}, Material: {after_layer['name']}, Thickness: {after_thickness}"
                    )
                else:
...
def CalculateAbsorption_tmm(
    dOpt, materialLayer, materialParams, lambda_=1064, t_air=500, polarisation="p"
):
    """
    ...
    t_coat = [
        dOpt[layer] for layer in materialLayer
    ]  # Assuming dOpt contains the thickness of each layer
    ...
    for i, thickness in enumerate(t_coat, start=1):  # start=1 skips the air layer
        absorption_in_layer = [
            absorption_fn(i, d) for d in np.linspace(0, thickness, num=100)
        ]
...
def CalculateTransmission_tmm(
    ...
    for lambda_ in lambda_list:
        ...
        n_list = (
            [complex(n_air, k_air)]
            + [complex(n_i, k_i) for n_i, k_i in zip(n_coat, k_coat)]
            + [complex(n_sub, k_sub)]
        )
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            try:
                ...
                coh_tmm_data = tmm.coh_tmm(
                    polarisation, n_list, d_list, angle * np.pi / 180, wavelength
                )
                T = coh_tmm_data["T"]
    ...
    if plots:
        ...
        data = {"Wavelength (nm)": wavelengths, "Transmission (%)": transmission * 100}
        ...
        fig.update_layout(
            ...
            xaxis=dict(showgrid=True, range=plot_range),  # Set the x-axis range
...
def CalculateTransmission_tmm2(
    dOpt,
    materialLayer,
    materialParams,
    lambda_list,
    ...
    for wnm in wavelengths:
        ...
        n_list = (
            [complex(n_air, k_air)]
            + [complex(a, b) for a, b in zip(n_coat, k_coat)]
            + [complex(n_sub, k_sub)]
        )
    ...
    if plots:
        ...
        df = pd.DataFrame(
            {"Wavelength (nm)": wavelengths, "Transmission (%)": transmission * 100}
        )
        fig = px.line(
            df,
            x="Wavelength (nm)",
...
def stack_RT_fast(
    ...
    if tphys is None:
        ...
        for layer_material in materialLayer:
            ...
            if verbose:
                print(
                    f"Layer {layer_material}: Optical Thickness: {dOpt[layer_material]}, Physical Thickness: {physical_thickness}, Refractive Index: {n_i}"
                )
            tphys.append(physical_thickness)
        tphys = np.array(tphys)
    else:
    ...
    if plots is not None:
        ...
        if plots:
            ...
            plt.plot(
                lambda_list,
                transmission_spec_scaled,
                label="Transmission Spectrum (Fast)",
            )
    ...
    return (
        wavelengths,
        transmission,
        transmission_lambda_0,
        {"refl": amplitude, "R": intensity, "T": T, "pol": polarisation},
    )
